...
static int asc_set_baud(struct uart_port *port, int baud, unsigned long clkrate);
...
{
	/* This routine is used for setting signals of: DTR, DCD, CTS/RTS
...
static void asc_set_termios(struct uart_port *port, struct ktermios *termios, struct ktermios *old)
{
...
static void __devinit asc_init_port(struct asc_port *ascport, struct platform_device *pdev)
{
...
static int asc_remap_port(struct asc_port *ascport, int req)
{
	...
	if (!ascport->pad_state)
	{
		...
		ascport->pad_state = stm_pad_claim(ascport->pad_config, "stasc");
		if (!ascport->pad_state)
		{
			return -EBUSY;
		}
...
static inline void asc_receive_chars(struct uart_port *port)
{
	...
	while (1)
	{
		...
		for (; count != 0; count--)
		{
			...
			else if (ascport->check_parity && unlikely(c & ASC_RXBUF_PE))
			{
				port->icount.parity++;
				flag = TTY_PARITY;
			}
...
static irqreturn_t asc_interrupt(int irq, void *ptr)
{
	...
	if (asc_fdma_enabled(port))
	{
		...
		{
			printk(KERN_ERR"Unknown ASC interrupt for port %p! (ASC_STA = %08x)\n", port, asc_in(port, STA));
		}
	}
	else
	{
...
static int asc_request_irq(struct uart_port *port)
{
	...
	if (request_irq(port->irq, asc_interrupt, 0, pdev->name, port))
	{
