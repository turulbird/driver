	...
	IN UCHAR FromWhichBSSID)
{
			...
			&&  (EAP_CODE_SUCCESS == WpaCheckEapCode(pAd, pRxBlk->pData, pRxBlk->DataSize, LENGTH_802_1_H)))
			{
				...
				if (pAd->StaCfg.IEEE8021x_required_keys == FALSE)
				{
					...
					if (pAd->StaCfg.DesireSharedKey[idx].KeyLen > 0)
					{
						...
						NdisMoveMemory(pAd->SharedKey[BSS0][idx].Key, pAd->StaCfg.DesireSharedKey[idx].Key, pAd->StaCfg.DesireSharedKey[idx].KeyLen);
					}
				}
			}
...
#endif /* WPA_SUPPLICANT_SUPPORT */
	{
		 ...
		 */
		{
			...
			REPORT_MGMT_FRAME_TO_MLME(pAd, pRxWI->WirelessCliID, pTmpBuf, pRxBlk->DataSize + LENGTH_802_11, pRxWI->RSSI0, pRxWI->RSSI1, pRxWI->RSSI2, 0, OPMODE_STA);
...
VOID STARxDataFrameAnnounce(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY *pEntry, IN RX_BLK *pRxBlk, IN UCHAR FromWhichBSSID)
{
	/* non-EAP frame */
...
BOOLEAN STACheckTkipMICValue(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY *pEntry, IN RX_BLK * pRxBlk)
{
	...
	if (RTMPTkipCompareMICValue(pAd, pData, pDA, pSA, pWpaKey->RxMic, UserPriority, DataSize) == FALSE)
	{
		...
		if (pAd->StaCfg.WpaSupplicantUP)
		{
			WpaSendMicFailureToWpaSupplicant(pAd->net_dev, (pWpaKey->Type == PAIRWISEKEY) ? TRUE : FALSE);
		}
		else
#endif /* WPA_SUPPLICANT_SUPPORT */
		{
...
//	FRAME_CONTROL *pFmeCtrl = &pHeader->FC;
	...
	if ((pHeader->FC.Wep == 1) && (pRxD->Decrypted == 0))
	{
		...
		if ((pSwKey = RTMPSwCipherKeySelection(pAd, pRxBlk->pData, pRxBlk, pEntry)) == NULL)
		{
		...
		if (RTMPSoftDecryptionAction(pAd, (PUCHAR) pHeader, UserPriority, pSwKey, pRxBlk->pData, &(pRxBlk->DataSize)) != NDIS_STATUS_SUCCESS)
		{
			/* release packet */
			RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_FAILURE);
			return;
...
BOOLEAN STARxDoneInterruptHandle(IN PRTMP_ADAPTER pAd, IN BOOLEAN argc)
{
	...
	while (1)
	{
		if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RADIO_OFF | fRTMP_ADAPTER_RESET_IN_PROGRESS | fRTMP_ADAPTER_HALT_IN_PROGRESS | fRTMP_ADAPTER_NIC_NOT_EXIST)
		...
		if (Status == NDIS_STATUS_SUCCESS)
		{
			...
			if (pAd->chipCap.FreqCalibrationSupport)
			{
			    	...
			    	&& (MAC_ADDR_EQUAL(&pAd->CommonCfg.Bssid, &pHeader->Addr2)))
				{
					...
					DBGPRINT(RT_DEBUG_INFO, ("%s: Beacon, CRC error = %d, pHeader->Sequence = %d, SA = %02X:%02X:%02X:%02X:%02X:%02X, frequency offset = %d, MCS = %d, BW = %d PHYMODE = %d\n",
...
BOOLEAN STAHandleRxDonePacket(IN PRTMP_ADAPTER pAd, IN PNDIS_PACKET pRxPacket, IN RX_BLK *pRxCell)
{
	RT28XX_RXD_STRUC *pRxD;
	PRXWI_STRUC pRxWI;
	PHEADER_802_11 pHeader;
	...
	IN PNDIS_PACKET pPacket)
{
	...
	/* Note multicast packets in adhoc also use BSSID_WCID index. */
	{
		if (pAd->StaCfg.BssType == BSS_INFRA)
		{
			...
			if (VALID_WCID(tmpWcid) && (IS_ENTRY_DLS(&pAd->MacTab.Content[tmpWcid]) || IS_ENTRY_TDLS(&pAd->MacTab.Content[tmpWcid])))
			{
	...
	&&  (pEntry->NoBADataCountDown == 0) && IS_HT_STA(pEntry))
	{
		...
		&&  ((IS_ENTRY_CLIENT(pEntry) && pAd->MlmeAux.APRalinkIe != 0x0) || (pEntry->WepStatus != Ndis802_11WEPEnabled && pEntry->WepStatus != Ndis802_11Encryption2Enabled))
...
VOID RTMPSendNullFrame(IN PRTMP_ADAPTER pAd, IN UCHAR TxRate, IN BOOLEAN bQosNull, IN USHORT PwrMgmt)
{
	UCHAR NullFrame[48];
	ULONG Length;
...
VOID RTMPP2PSendNullFrame(IN PRTMP_ADAPTER pAd, IN UCHAR TxRate, IN BOOLEAN bQosNull, IN USHORT PwrMgmt)
{
	UCHAR NullFrame[48];
	ULONG Length;
	PHEADER_802_11 pHeader_802_11;
...

	{
		...
		if (KeyIdx == 0xff)
		{
			...
			|| (((Cipher == Ndis802_11Encryption1Enabled) && (pAd->SharedKey[BSS0][KeyIdx].KeyLen == 0))
		        ...
		        && pMacEntry
		        && (pMacEntry->PairwiseKey.KeyLen == 0))
		        && (!ADHOC_ON(pAd))))
...
VOID STA_AMPDU_Frame_Tx(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk)
{
	...
	while (pTxBlk->TxPacketList.Head)
	{
...
#endif /* VENDOR_FEATURE1_SUPPORT */
		{
			...
			*(pHeaderBufPtr + 1) = 0;
			...
			if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bSwEncrypt))
			{
				...
				RTMPSoftConstructIVHdr(pTxBlk->CipherAlg, pTxBlk->KeyIdx, pTxBlk->pKey->TxTsc, pHeaderBufPtr, &iv_offset);
				pHeaderBufPtr += iv_offset;
				pTxBlk->MpduHeaderLen += iv_offset;

				/* Encrypt the MPDU data by software */
				RTMPSoftEncryptionAction(pAd, pTxBlk->CipherAlg, (PUCHAR)pHeader_802_11, pTxBlk->pSrcBufData, pTxBlk->SrcBufLen,
		...
		else
		{
			...
			NdisMoveMemory((PUCHAR) (&pMacEntry->CachedBuf[0]), (PUCHAR) (&pTxBlk->HeaderBuf[TXINFO_SIZE]),
			...
			NdisMoveMemory((PUCHAR) (&pMacEntry->HeaderBuf[0]), (PUCHAR) (&pTxBlk->HeaderBuf[0]),
	...
	do
	{
		...
		if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bSwEncrypt))
		{
			...
			|| (pTxBlk->CipherAlg == CIPHER_WEP128))
			{
				...
				RTMPConstructWEPIVHdr(pTxBlk->KeyIdx, pTxBlk->pKey->TxTsc, pHeaderBufPtr - (LEN_WEP_IV_HDR));
			}
			else if (pTxBlk->CipherAlg == CIPHER_TKIP);
			else if (pTxBlk->CipherAlg == CIPHER_AES)
			{
...
NDIS_STATUS STAHardTransmit(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk, IN UCHAR QueIdx)
{
	...
	if ((!(pAd->StaCfg.UapsdInfo.bAPSDCapable && pAd->CommonCfg.APEdcaParm.bAPSDCapable) && (pAd->CommonCfg.bAPSDForcePowerSave == FALSE))
...
VOID Sta_Announce_or_Forward_802_3_Packet(IN PRTMP_ADAPTER pAd, IN PNDIS_PACKET pPacket, IN UCHAR FromWhichBSSID)
{
	if (TRUE)
	{
