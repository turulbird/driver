...
NDIS_STATUS AsicBBPWriteWithRxChain(IN RTMP_ADAPTER *pAd, IN UCHAR bbpId, IN CHAR bbpVal, IN RX_CHAIN_IDX rx_ch_idx)
{
	UCHAR idx = 0, val = 0;
	...
	while (rx_ch_idx != 0)
	{
		...
		if (rx_ch_idx & 0x01)
		{
			...
			DBGPRINT(RT_DEBUG_INFO, ("%s(Idx):Write(R%d,val:0x%x) to Chain(0x%x, idx:%d)\n", __FUNCTION__, bbpId, bbpVal, rx_ch_idx, idx));
		}
		rx_ch_idx >>= 1;
		idx++;
	}
...
NDIS_STATUS AsicBBPReadWithRxChain(IN RTMP_ADAPTER *pAd, IN UCHAR bbpId, IN CHAR *pBbpVal, IN RX_CHAIN_IDX rx_ch_idx)
{
	UCHAR idx, val;
	...
	while(rx_ch_idx != 0)
	{
		...
		if (rx_ch_idx & 0x01)
		{
			...
			val = (val & (~0x60)) | (idx << 5);
#ifdef RTMP_MAC_USB
			if ((IS_USB_INF(pAd)) && (RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R27, val) == STATUS_SUCCESS))
			{
				RTMP_BBP_IO_READ8_BY_REG_ID(pAd, bbpId, pBbpVal);
			}
#endif /* RTMP_MAC_USB */
...
#ifdef MT7601
NDIS_STATUS MT7601_BBP_write(IN PRTMP_ADAPTER pAd, IN UCHAR regID, IN UCHAR value)
{
...
NDIS_STATUS MT7601_BBP_read(IN RTMP_ADAPTER *pAd, IN UCHAR regID, IN UCHAR *pValue)
{
	BBP_CSR_CFG_STRUC BbpCsr = { { 0 } };
	UINT i = 0, k = 0;
	...
	for (i = 0; i < MAX_BUSY_COUNT; i++)
	{
		...
		if ((BbpCsr.field.Busy == IDLE) && (BbpCsr.field.RegNum == regID))
		{
