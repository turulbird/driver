...
{
	dprintk(20, "[AK4705]: SAAIOSMODE command : %d\n", __func__, vol);
	if (vol == SAA_MODE_RGB)
	{
...
/* ---------------------------------------------------------------------- */
//NOT IMPLEMENTED
int ak4705_set_encoder(struct i2c_client *client, int val)
{
	return 0;
}
...
int ak4705_set_wss(struct i2c_client *client, int val)
{
	if (val == SAA_WSS_43F)
	{
...
int ak4705_command(struct i2c_client *client, unsigned int cmd, void *arg)
{
	...
	if (cmd & AVSIOSET)
	{
		...
		dprintk(20, "[AK4705] %s: AVSIOSET command\n", __func__);
		...
		switch (cmd)
		{
			...
			default:
			{
				dprintk(1, "[AK4705] %s: AVSIOSET command 0x%02x not supported\n", __func__, cmd);
				return -EINVAL;
			}
		}
	}
	...
	{
		dprintk(20, "[AK4705] %s: AVSIOGET command\n", __func__);
		...
		switch (cmd)
		{
			...
			default:
			{
				dprintk(1, "[AK4705] %s: AVSIOGET command 0x%02x not supported\n", __func__, cmd);
				return -EINVAL;
			}
		}
	...
	{
		dprintk(20, "[AK4705] %s: SAA command\n", __func__);

		if (copy_from_user(&val, arg, sizeof(val)))
		{
			dprintk(1, "[AK4705] %s: SAA command - fault\n", __func__);
			return -EFAULT;
		}
		...
		switch (cmd)
		{
			...
			default:
			{
				dprintk(1, "[AK4705] %s: SAA command 0x%02x not supported\n", __func__, cmd);
				return -EINVAL;
			}
		}
	}
...
int ak4705_command_kernel(struct i2c_client *client, unsigned int cmd, void *arg)
{
	...
	dprintk(20, "[AK4705] %s: command_kernel (%u)\n", __func__, cmd);
	...
	else
	{
		...
		switch (cmd)
		{
			...
			default:
			{
				dprintk(1, "[AK4705] %s: SAA command not 0x%02x supported\n", __func__, cmd);
				return -EINVAL;
			}
		}
	}
