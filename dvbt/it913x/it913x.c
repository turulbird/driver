 ...
 *
 * Adapted to DVBAPI version 5 and SH4 by Audioniek.
 *
...
static const char Revision[] = "1.28";
...
static int it913x_bulk_write(struct usb_device *dev, u8 *snd, int len, u8 pipe)
{
	int ret, actual_l, i;

	for (i = 0; i < IT913X_RETRY; i++)
	{
		ret = usb_bulk_msg(dev, usb_sndbulkpipe(dev, pipe), snd, len , &actual_l, IT913X_SND_TIMEOUT);
...
static int it913x_bulk_read(struct usb_device *dev, u8 *rev, int len, u8 pipe)
{
	int ret, actual_l, i;

	for (i = 0; i < IT913X_RETRY; i++)
	{
		ret = usb_bulk_msg(dev, usb_rcvbulkpipe(dev, pipe), rev, len , &actual_l, IT913X_RCV_TIMEOUT);
		if (ret != -EBUSY && ret != -ETIMEDOUT)
		{
			break;
		}
...
static int it913x_usb_talk(struct usb_device *udev, u8 mode, u8 pro, u8 cmd, u32 reg, u8 addr, u8 *data, u8 len)
{
	...
	ret = it913x_bulk_read(udev, buff, (mode & 1) ? 5 : len + 5 , 0x01);
	if (ret < 0)
	{
		goto error;
	}
...
static int it913x_io(struct usb_device *udev, u8 mode, u8 pro, u8 cmd, u32 reg, u8 addr, u8 *data, u8 len)
{
	int ret, i;

	for (i = 0; i < IT913X_RETRY; i++)
	{
		ret = it913x_usb_talk(udev, mode, pro, cmd, reg, addr, data, len);
		if (ret != -EAGAIN)
		{
			break;
		}
...
static int it913x_wr_reg(struct usb_device *udev, u8 pro, u32 reg , u8 data)
{
	...
	ret = it913x_io(udev, WRITE_LONG, pro, CMD_DEMOD_WRITE, reg, 0, b, sizeof(b));
...
static int it913x_read_reg(struct usb_device *udev, u32 reg)
{
	...
	ret = it913x_io(udev, READ_LONG, DEV_0, CMD_DEMOD_READ, reg, 0, &data[0], 1);
...
static u32 it913x_query(struct usb_device *udev, u8 pro)
{
	...
	for (i = 0; i < 5; i++)
	{
		ret = it913x_io(udev, READ_LONG, pro, CMD_DEMOD_READ, 0x1222, 0, &data[0], 3);
		ver = data[0];
	...
	info("Chip: Version=%02x, Type=%04x", it913x_config.chip_ver, it913x_config.chip_type);

	ret |= it913x_io(udev, READ_SHORT, pro, CMD_QUERYINFO, 0, 0x1, &data[0], 4);

	it913x_config.firmware = (data[0] << 24) + (data[1] << 16) + (data[2] << 8) + data[3];
...
static int it913x_return_status(struct usb_device *udev)
{
	...
	if (firm > 0)
	{
		info("Firmware Version %x", firm);
	}
	return (firm > 0) ? firm : 0;
}

static int it913x_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msg[], int num)
{
...
//	debug_data_snipet(1, "Message out", msg[0].buf);
	...
	if (num == 2)
	{
		ret = it913x_io(d->udev, READ_LONG, pro, CMD_DEMOD_READ, reg, 0, data, msg[1].len);
		memcpy(msg[1].buf, data, msg[1].len);
	}
	else
	{
		ret = it913x_io(d->udev, WRITE_LONG, pro, CMD_DEMOD_WRITE, reg, 0, &data[4], msg[0].len - 4);
	}
...
static int it913x_rc_query(struct dvb_usb_device *d)
{
	...
	ret = it913x_io(d->udev, READ_LONG, PRO_LINK, CMD_IR_GET, 0, 0, &ibuf[0], sizeof(ibuf));
...
static int ite_firmware_select(struct usb_device *udev, struct dvb_usb_device_properties *props)
{
	int sw;
...
#if defined RC_ENABLE
static void it913x_select_remote(struct usb_device *udev, struct dvb_usb_device_properties *props)
{
...
static int it913x_select_config(struct usb_device *udev, struct dvb_usb_device_properties *props)
{
	int ret = 0, reg;
	bool proprietary_ir = false;

	if (it913x_config.chip_ver == 0x02 && it913x_config.chip_type == 0x9135)
	{
...
//		props->rc.core.rc_codes = NULL;
	...
	info("Dual mode=%x Tuner Type=%x", it913x_config.dual_mode, it913x_config.tuner_id_0);
	...
	if (udev->speed != USB_SPEED_HIGH)
	{
		...
		info("USB 1.1 low speed mode - connect to USB 2.0 port");
		if (pid_filter > 0)
		{
			pid_filter = 0;
		}
		...
		{
			it913x_config.dual_mode = 0;
			info("Dual mode not supported in USB 1.1");
		}
	}
	else /* For replugging */
	...
	if (pid_filter)
	{
		props->adapter[0].stream.u.bulk.buffersize = TS_BUFFER_SIZE_MAX;
		props->adapter[0].caps &= ~DVB_USB_ADAP_NEED_PID_FILTERING;
	}
	else
	{
		props->adapter[0].stream.u.bulk.buffersize = TS_BUFFER_SIZE_PID;
	}
	if (it913x_config.dual_mode)
	{
		props->adapter[1].stream.u.bulk.buffersize = props->adapter[0].stream.u.bulk.buffersize;
		props->num_adapters = 2;
		if (pid_filter)
		{
			props->adapter[1].caps = props->adapter[0].caps;
		}
	}
	else
	{
	...
	info("Tuner: Dual mode=%x Type=%x", it913x_config.dual_mode, it913x_config.tuner_id_0);
...
static int it913x_download_firmware(struct usb_device *udev, const struct firmware *fw)
{
	int ret = 0, i = 0, pos = 0;
	...
	while (i <= fw->size)
	{
		if (((fw->data[i] == 0x3) && (fw->data[i + 2] == 0x0)) || (i == fw->size))
		{
			packet_size = i - pos;
			if ((packet_size > min_pkt) || (i == fw->size))
			{
				...
				if (packet_size > 0)
				{
					ret = it913x_io(udev, WRITE_DATA, DEV_0, CMD_SCATTER_WRITE, 0, 0, fw_data, packet_size);
					if (ret < 0)
					{
						break;
					}
	...
	if ((it913x_config.chip_ver == 1) && (it913x_config.chip_type == 0x9135))
	{
...
static int it913x_frontend_attach(struct dvb_usb_adapter *adap)
{
	...
	if (adap->id == 0)
	{
		memcpy(&st->it913x_config, &it913x_config, sizeof(struct ite_config));
	}
	adap->fe = dvb_attach(it913x_fe_attach, &adap->dev->i2c_adap, adap_addr, &st->it913x_config);
	...
	if (adap->id == 0 && adap->fe)
	{
		...
		ret = it913x_wr_reg(udev, DEV_0, EP4_TX_LEN_LSB, ep_size & 0xff);
	...
	else if (adap->id == 1 && adap->fe)
	{
		...
		ret = it913x_wr_reg(udev, DEV_0, EP5_TX_LEN_LSB, ep_size & 0xff);
...
static int it913x_tuner_attach(struct dvb_usb_adapter *adap)
{
	return 0;
}
...
static int it913x_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	cmd_counter = 0;

	info("IT913X USB2.0 driver version %s initializing", Revision);

	if (0 == dvb_usb_device_init(intf, &it913x_properties, THIS_MODULE, NULL, adapter_nr))
	{
		info("DEV registering device driver");
		return 0;
	}
	info("DEV it913x Initializing USB device failed");
	return -ENODEV;
}
...
static struct usb_device_id it913x_table[] = {
	...
	{ USB_DEVICE(USB_VID_AVERMEDIA, USB_PID_AVERMEDIA_A835B_1835) },
	{ USB_DEVICE(USB_VID_AVERMEDIA, USB_PID_AVERMEDIA_A835B_2835) },
	{ USB_DEVICE(USB_VID_AVERMEDIA, USB_PID_AVERMEDIA_A835B_3835) },
	{ USB_DEVICE(USB_VID_AVERMEDIA, USB_PID_AVERMEDIA_A835B_4835) },
	{ USB_DEVICE(USB_VID_KWORLD_2, USB_PID_CTVDIGDUAL_V2) },
	{ USB_DEVICE(USB_VID_AVERMEDIA, USB_PID_AVERMEDIA_H335) },
	{}		/* Terminating entry */
};
	...
	{
		{
			...
			.tuner_attach     = it913x_tuner_attach,
			.stream =
			{
				.type = USB_BULK,
				.count = 10,
				.endpoint = 0x04,
				.u =
				{/* Keep Low if PID filter on */
					.bulk =
					{
						.buffersize = TS_BUFFER_SIZE_PID,
					}
				}
			},
//			.size_of_priv;
...
//	.power_ctrl = it913x_power_ctrl,
//	.read_mac_address =
	.identify_state = it913x_identify_state,
#if defined RC_ENABLE
	.rc_key_map = RC_MAP_IT913X_V1,
	.rc_key_map_size = sizeof(RC_MAP_IT913X_V1),
	...
	.devices = {
		...
		{ "ITE 9135 Generic", { &it913x_table[1], NULL }, },
		{ "Sveon STV22 Dual DVB-T HDTV(IT9137)", { &it913x_table[2], NULL }, },
		{ "ITE 9135(9005) Generic", { &it913x_table[3], NULL }, },
		{ "ITE 9135(9006) Generic", { &it913x_table[4], NULL }, },
		{ "Avermedia A835B(1835)", { &it913x_table[5], NULL }, },
		{ "Avermedia A835B(2835)", { &it913x_table[6], NULL }, },
		{ "Avermedia A835B(3835)", { &it913x_table[7], NULL }, },
		{ "Avermedia A835B(4835)", { &it913x_table[8], NULL }, },
		{ "Digital Dual TV Receiver CTVDIGDUAL_V2", { &it913x_table[9], NULL }, },
		{ "Avermedia H335", { &it913x_table[10], NULL }, },
	}
};
...
static int __init it913x_init(void)
{
	return usb_register(&it913x_driver);
}

static void __exit it913x_exit(void)
{
	usb_deregister(&it913x_driver);
}

module_init(it913x_init);
module_exit(it913x_exit);

MODULE_AUTHOR("Malcolm Priestley <tvboxspy@gmail.com>");
MODULE_DESCRIPTION("it913x USB 2 Driver");
MODULE_VERSION("1.28_SH4");
MODULE_LICENSE("GPL");
MODULE_PARM_DESC(debug, "Set debugging level (1=info (or-able))." DVB_USB_DEBUG_STATUS);
MODULE_PARM_DESC(pid, "Set default 0=on 1=off");
MODULE_PARM_DESC(firmware, "Set firmware 0=auto 1=IT9137 2=IT9135V1 3=IT9135V2");
// vim:ts=4
