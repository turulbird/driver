 ...
 *
 *  with support for on chip IT9135 & IT9137 integral tuner
 *
...
static int it913x_debug_fe;

module_param_named(debug_fe, it913x_debug_fe, int, 0644);

#define dprintk(level, args...) do { \
	if (level & it913x_debug_fe) \
...
static int it913x_read_reg(struct it913x_fe_state *state, u32 reg, u8 *data, u8 count)
{
	...
	struct i2c_msg msg[2] = {
		{ .addr = state->i2c_addr + (pro << 1), .flags = 0, .buf = b, .len = sizeof(b) },
		{ .addr = state->i2c_addr + (pro << 1), .flags = I2C_M_RD, .buf = data, .len = count }
	};
	b[0] = (u8) reg >> 24;
	b[1] = (u8)(reg >> 16) & 0xff;
	b[2] = (u8)(reg >> 8) & 0xff;
...
static int it913x_write(struct it913x_fe_state *state, u8 pro, u32 reg, u8 buf[], u8 count)
{
	u8 b[256];
	struct i2c_msg msg[1] = {
		{ .addr = state->i2c_addr + (pro << 1), .flags = 0, .buf = b, .len = count + 4 }
	};
	int ret;
...
static int it913x_fe_script_loader(struct it913x_fe_state *state, struct it913xset *loadscript)
{
	...
	for (i = 0; i < 1000; ++i)
	{
		...
		ret = it913x_write(state, loadscript[i].pro, loadscript[i].address, loadscript[i].reg, loadscript[i].count);
...
static int it913x_fe_set_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters* params)
{
	...
	switch (state->tuner_type)
	{
		...
		case IT9135_62:
		{
			it9137_set_tuner(state, p->bandwidth_hz, p->frequency);
			break;
		}
		default:
		{
...
struct dvb_frontend *it913x_fe_attach(struct i2c_adapter *i2c_adap, u8 i2c_addr, struct ite_config *config)
{
	...
	/* create dvb_frontend */
	memcpy(&state->frontend.ops, &it913x_fe_ofdm_ops, sizeof(struct dvb_frontend_ops));
...
MODULE_VERSION("1.15");
MODULE_LICENSE("GPL");
MODULE_PARM_DESC(debug_fe, "set debugging level (1=info (or-able)).");
// vim:ts=4
