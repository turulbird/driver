...
static u32 stv090x_get_srate(struct stv090x_state *state, u32 clk)
{
	...
	dprintk(20, "Locked with symbol rate %d.%d MS/s\n", srate/ 1000, srate % 1000);
	dprintk(100, "%s <\n", __func__);
	return srate;
}
...
static int stv090x_get_dmdlock(struct stv090x_state *state, s32 timeout)
{
	...
	while ((timer < timeout) && (!lock))
	{
		...
		{
			dprintk(150, "%s: Demodulator acquired LOCK\n", __func__);
		}
		timer += 10;
	}
	if (lock)
...
static int stv090x_chk_signal(struct stv090x_state *state)
{
	...
	dprintk(50, "%s < Signal %s\n", __func__, (no_signal ? "no" : "yes"));
	return no_signal;
}
...
static int stv090x_search_car_loop(struct stv090x_state *state, s32 inc, s32 timeout, int zigzag, s32 steps_max)
{
	...
	dprintk(100, "%s >\n", __func__);
	dprintk(70, "%s inc %d, timeout %d, zigzag %d, setps_max %d\n", __func__, inc, timeout, zigzag, steps_max);
...
static u8 stv090x_optimize_carloop(struct stv090x_state *state, enum stv090x_modcod modcod, s32 pilots)
{
	...
	{
		dprintk(150, "%s STX7111 cut 0x20 handling (modcod %d, pilots %d)\n", __func__, modcod, pilots);
...
static int stv090x_optimize_track(struct stv090x_state *state)
{
	...
	switch (state->delsys)
	{
		...
		default:
		{
			dprintk(50, "STV090x: Unkown delivery system\n");
			reg = STV090x_READ_DEMOD(state, DMDCFGMD);
			STV090x_SETFIELD_Px(reg, DVBS1_ENABLE_FIELD, 1);
			STV090x_SETFIELD_Px(reg, DVBS2_ENABLE_FIELD, 1);
			if (STV090x_WRITE_DEMOD(state, DMDCFGMD, reg) < 0)
	...
	if ((state->dev_ver >= 0x20) || (blind_tune == 1) || (state->srate < 10000000))
	{
		...
		dprintk(70, "%s: f_1 0x%x\n", __func__, f_1);
		dprintk(70, "%s: f_0 0x%x\n", __func__, f_0);
	...
	while ((timer < timeout) && (!lock))
	{
		...
		switch (stat)
		{
			...
			case 2: /* DVB-S2 mode */
			{
				...
				dprintk(50, "%s DVB-S2 mode: reg = 0x%x, lock = %d\n", __func__, reg, lock);
				break;
			}
			case 3: /* DVB-S1/legacy mode */
			{
				...
				dprintk(50, "%s DVB-S mode: reg = 0x%x, lock = %d\n", __func__, reg, lock);
				break;
			}
		}
		if (!lock)
...
static enum dvbfe_search stv090x_search(struct dvb_frontend *fe, struct dvbfe_params *p)
{
	...
	dprintk(100, "%s >\n", __func__);
	dprintk(50, "%s: freq %d, symbol %d, inversion %d, rolloff %d, modulation %d, fec %d, delsys %d\n", __func__,
	...
	if (state->srate > 10000000)
	{
		dprintk(30, "Search range: 10 MHz\n");
		state->search_range = 10000000;
	}
	else
	{
		dprintk(30, "Search range: 5 MHz\n");
		state->search_range = 5000000;
	}
	algo_state = stv090x_algo(state);
	if (algo_state == STV090x_RANGEOK)
	{
		dprintk(30, "Search success!\n");
...
static int stv090x_set_pls(struct stv090x_state *state, u8 pls_mode, u32 pls_code)
{
	...
	pls_mode &= 0x03;
	pls_code &= 0x3FFFF;
	dprintk(50, "Set PLS code %d (mode %d)", pls_code, pls_mode);
...
static int stv090x_set_mis(struct stv090x_state *state, int mis)
{
	...
	{
		dprintk(50, "Disable MIS filtering");
	...
	{
		dprintk(50, "Enable MIS filtering - %d", mis);
...
static int stv090x_read_signal_strength(struct dvb_frontend *fe, u16 *strength)
{
	...
	*/
//TDT
	agc = MAKEWORD16(STV090x_READ_DEMOD(state, AGCIQIN1), STV090x_READ_DEMOD(state, AGCIQIN0));
	...
	switch (ldpc_mode)
	{
		...
		default:
		{
			if ((state->demod_mode != STV090x_DUAL) || (STV090x_GETFIELD(reg, DDEMOD_FIELD) != 1))
			{
				...
				dprintk(50, "setting to dual mode\n");
		...
		case STV090x_SINGLE:
		{
			dprintk(50, "setting to single mode\n");
...
static int stv090x_set_mclk(struct stv090x_state *state, u32 mclk, u32 clk)
{
	...
	if (state->device == STX7111)
	{
		...
		dprintk(50, "%s: reading the masterclock = %d\n", __func__, state->mclk);
	}
	else
	{
		reg = stv090x_read_reg(state, STV090x_SYNTCTRL);
...
static int stv090x_init(struct dvb_frontend *fe)
{
	...
	if (state->device == STX7111)
	{
		dprintk(20, "Found %s Demodulator, cut %02x\n", "STV090x(STX711x)", state->dev_ver);
	}
	else
	{
		dprintk(20, "Found %s Demodulator, Cut %02x\n", state->device == STV0900 ? "STV0900" : "STV0903",
...
static int stv090x_setup(struct dvb_frontend *fe)
{
	...
	{
		dprintk(20, "Initializing STX7111\n");
...
{
	.name                     = "STM STV090x Multistandard",
...
static int spark_set_voltage(struct dvb_frontend *fe, enum fe_sec_voltage voltage)
{
	...
	switch (voltage)
	{
		...
		{
			dprintk(20, "Set_LNB voltage 13V (vertical)\n");
				    ...
				    enum stv090x_tuner tuner)
{
	...
	state->tuner                     = tuner;
	dprintk(70, "%s i2c adapter = %p, i2c addr = 0x%02x\n", __func__, state->i2c, config->address);
	...
	{
		dprintk(1, "Request for LNB power control PIO failed\n");
		if (fe_lnb_13_18)
		{
	...
	if (state->device == STX7111)
	{
		...
		dprintk(20, "Attaching %s demodulator(%d) cut %02x\n", "STV090x(STX711x)", demod, state->dev_ver);
	}
	else
	{
		if (state->dev_ver >= 0x30)
