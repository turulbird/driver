/**************************************************************************
 *
 * STM STV6110(A) Silicon tuner driver
 *
 * Copyright (C) Manu Abraham <abraham.manu@gmail.com>
 *
 * Copyright (C) ST Microelectronics
 *
 * Version for Fulan Spark
 *
...
static int stv6110x_read_reg(struct stv6110x_state *stv6110x, u8 reg, u8 *data)
{
	...
	*data = b1[0];
	...
	dprintk(100, "%s < reg=0x%02x , data=%02x\n", __func__, reg, *data);
	return 0;
}
...
static int stv6110x_init(struct dvb_frontend *fe)
{
	...
	dprintk(100, "%s > i2c-%d\n", __func__, stv6110x->i2c->nr);

	ret = stv6110x_write_init(stv6110x);
	if (ret < 0)
	{
		dprintk(1, "%s: stv6110x_init Initialization failed\n", __func__);
		return -1;
	}
	stv6110x->gain = 10;
...
static int stv6110x_set_frequency(struct dvb_frontend *fe, u32 frequency)
{
	...
	dprintk(100, "%s >\n", __func__);
	dprintk(20, "%s, Frequency = %d kHz, mclk=%d Hz\n", __func__, frequency, stv6110x->config->refclk);

	/* K = (Reference / 1000000) - 16 */
	stv6110x_regs[STV6110x_CTRL1] &= ~(0x1f << 3);
	stv6110x_regs[STV6110x_CTRL1] |= ((((stv6110x->config->refclk / 1000000) - 16) & 0x1f) << 3);
...
struct tuner_devctl *stv6110x_attach(struct dvb_frontend *fe, const struct stv6110x_config *config,
				struct i2c_adapter *i2c)
{
	...
	dprintk(20, "Attaching STM STV6110x tuner\n", __func__);
