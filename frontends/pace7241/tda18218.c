...
static int tda18218_wr_regs(struct tda18218_priv *priv, u8 reg, u8 *val, u8 len)
{
	...
	{
		dprintk(1, "%s: Error, ret:%d reg:%02x len:%d", __func__, ret, reg, len);
		ret = -EREMOTEIO;
	}
	return ret;
}
...
int tda18218_set_params(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
...
error:
	...
	if (ret)
	{
		dprintk(1, "%s: Error, ret:%d", __func__, ret);
	}
	return ret;
}

#if 0 // not available in Linux 2.6.32
static int tda18218_get_if_frequency(struct dvb_frontend *fe, u32 *frequency)
{
	struct tda18218_priv *priv = fe->tuner_priv;

	*frequency = priv->if_frequency;
	dprintk(20, "%s: if = %d", __func__, *frequency);
	return 0;
}
#endif
...
struct dvb_frontend *tda18218_attach(struct dvb_frontend *fe, struct i2c_adapter *i2c, struct tda18218_config *cfg)
{
	...
	dprintk(20, "%s: ret:%d chip ID:%02x", __func__, ret, val);
	if (ret || val != def_regs[R00_ID])
	{
		kfree(priv);
		return NULL;
	}
	dprintk(20, "NXP TDA18218HN successfully identified.");

	memcpy(&fe->ops.tuner_ops, &tda18218_tuner_ops, sizeof(struct dvb_tuner_ops));
