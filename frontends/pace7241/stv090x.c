 ...
 *  Pace HDS7421/91
 *
 * STV0900/STV0903 with STM STV6110x tuner,
 * LNB power control built around an Allegro A8292 LNB power
 * contoller, driven through I2C from this driver.
 * Tuner is controlled directly through I2C, not via
 * the I2C repeater of /STV0900/STV0903.
 * 
...
static struct stv090x_reg stv0903_initval[] =
{
	...
	{ STV090x_AGCRF1CFG,      0x11 },  // 0x11 for stv6110
...
static int stv090x_read_reg(struct stv090x_state *state, unsigned int reg)
{
	...
	if (unlikely(paramDebug >= 150))
	{
		dprintk(1, "%s Reg=[0x%02x], data=0x%02x\n", __func__, reg, buf);
	}
...
static int stv090x_write_regs(struct stv090x_state *state, unsigned int reg, u8 *data, u32 count)
{
	...
	if (unlikely(paramDebug >= 150))
	{
		...
		dprintk(1, "%s [0x%04x]:", __func__, reg);
		for (i = 0; i < count; i++)
		{
			printk(" %02x", data[i]);
		}
...
static int stv090x_i2c_gate_ctrl(struct dvb_frontend *fe, int enable)
{
	...
	return 0;  // I2C gate is not used on this receiver

#if 1  // never reached!
	if (state->demod == STV090x_DEMODULATOR_0) 
	{
		dprintk(20, "!!!!!!! STV090x_DEMODULATOR_0 gate:\n");
	}
	else
	{
		dprintk(20, "!!!!!!! STV090x_DEMODULATOR_1 gate:\n");
	}
	//--------------------------------------------------------	
	if (enable)
	{
...
static void stv090x_get_lock_tmg(struct stv090x_state *state)
{
	switch (state->algo)
	{
		...
		{
			dprintk(20, "Blind Search");
			if (state->srate <= 1500000)  /* 10Msps < SR <= 15Msps */
			{
				state->DemodTimeout = 1500;
				state->FecTimeout = 400;
			}
			else if (state->srate <= 5000000)  /* 15Msps < SR <= 50Msps */
			{
...
static int stv090x_vitclk_ctl(struct stv090x_state *state, int enable)
{
	...
	switch (state->demod)
	{
		...
		default:
		{
			dprintk(1, "Invalid demodulator # %d!\n", state->demod);
			break;
		}
	}
	return 0;
...
static int stv090x_get_dmdlock(struct stv090x_state *state, s32 timeout)
{
	...
	while ((timer < timeout) && (!lock))
	{
		...
		switch (stat)
		{
			...
			default:
			{
				dprintk(50, "Demodulator searching...\n");
				lock = 0;
				break;
			}
			case 2: /* DVB-S2 mode */
		...
		{
			dprintk(20, "Demodulator acquired LOCK\n");
		}
		timer += 10;
	}
	return lock;
...
static enum stv090x_signal_state stv090x_algo(struct stv090x_state *state)
{
	...
	{
		dprintk(10, "No Signal: POWER_IQ=0x%02x\n", power_iq);
		lock = 0;
...
static int stv090x_set_tone(struct dvb_frontend *fe, fe_sec_tone_mode_t tone)
{
	...
	switch (tone)
	{
		...
		case SEC_TONE_OFF:
		{
			dprintk(10, "Frontend %d: Tone off\n", id);
			STV090x_SETFIELD_Px(reg, DISTX_MODE_FIELD, 0);
			STV090x_SETFIELD_Px(reg, DISEQC_RESET_FIELD, 1);
			if (STV090x_WRITE_DEMOD(state, DISTXCTL, reg) < 0)
			{
...
static int stv090x_set_voltage(struct dvb_frontend *fe, fe_sec_voltage_t voltage)
{
	...
	u8 b;
	int ret, id = 0;
	struct i2c_msg msg = {.addr = 0x0b, .flags = 0, .buf = &b, .len = 1 };
	...
	switch (voltage)
	{
		case SEC_VOLTAGE_13:
		{
			...
			if (id == 0)
			{
				...
				if (ret = !1)
				{
					dprintk(1, "SEC_VOLTAGE_13 Error A8292\n");
				}
			}
			else
			{
			...
			if (ret != 1)
			{
				dprintk(1, "%s: Error setting LNB voltage to 13V (frontend %d)\n", __func__, id);
			}
			break;
		}
		case SEC_VOLTAGE_18:
		{
			...
			if (id == 0)
			{
				...
				if (ret = !1)
				{
					dprintk(1, "SEC_VOLTAGE_18 Error A8292\n");
				}
			}
			else
			{
		...
		{
			dprintk(10, "Frontend %d: Voltage off\n", id);
			if (state->demod == STV090x_DEMODULATOR_0)
			{
				b = 0x10;  // lnb1	34	3b
				ret = i2c_transfer(state->i2c, &msg, 1);
				if (ret = !1)
				{
					dprintk(1, "SEC_VOLTAGE_OFF Error A8292\n");
				}
			}
			else
			{
...
static int stv090x_send_diseqc_msg(struct dvb_frontend *fe, struct dvb_diseqc_master_cmd *cmd)
{
	...
	dprintk(10, "%s demodulator %d - length = %d\n", __func__, id, cmd->msg_len);

	if (paramDebug >= 50)
	{
		dprintk(10, "%s DiSEqC msg = 0x", __func__);
		for (i = 0; i < cmd->msg_len; i++)
		{
			printk("%02x", cmd->msg[i]);
		}
		printk("\n");
	}
	reg = STV090x_READ_DEMOD(state, DISTXCTL);
...
static int stv090x_send_diseqc_burst(struct dvb_frontend *fe, fe_sec_mini_cmd_t burst)
{
	...
	if (burst == SEC_MINI_A)
	{
		dprintk(10, "Frontend %d: DiSEqC Mini A\n", id);
		mode = (state->config->diseqc_envelope_mode) ? 5 : 3;
		value = 0x00;
	}
	else
	{
		dprintk(10, "Frontend %d: DiSEqC Mini B\n", id);
...
static int stv090x_wakeup(struct dvb_frontend *fe)
{
	...
	dprintk(20, "Wake %s from standby\n", state->device == STV0900 ? "STV0900" : "STV0903");
...
static void stv090x_release(struct dvb_frontend *fe)
{
	...
	{
		dprintk(20, "%s Actually removing\n", __func__);
...
static int stv090x_set_tspath(struct stv090x_state *state)
{
	...
	// fix for cut3.0 - 18.11.2012 (SagemCom ESI88)
	if (state->internal->dev_ver >= 0x30)
	{
		if ((state->config->ts1_mode == STV090x_TSMODE_SERIAL_CONTINUOUS)
		&&  (state->config->ts2_mode == STV090x_TSMODE_SERIAL_CONTINUOUS))
		{
	...
	if (state->config->ts1_clk > 0)
	{
		...
		switch (state->config->ts1_mode)
		{
			...
			default:
			{
				speed = state->internal->mclk / (state->config->ts1_clk / 4);
				if (speed < 0x08)
				{
					speed = 0x08;
				}
...
static int stv090x_init(struct dvb_frontend *fe)
{
	...
	int ret;
	struct i2c_msg msg =   {.addr = 0x0b, .flags = 0, .buf = &b, .len = 1 };
	struct i2c_msg msg_r = {.addr = 0x0b, I2C_M_RD,   .buf = &b, .len = 1 };
	...
	if (first_init == 0)
	{
		...
		if (ret != 1)
		{
			dprintk(1, "%s: Error:LNB init1 A8292\n", __func__);
		}
		b = 0x10;  // lnb1
		ret = i2c_transfer(state->i2c, &msg, 1);
		if (ret != 1)
		{
			dprintk(1, "%s: Error: LNB1 init2 A8292\n", __func__);
		}
		b = 0x10 + 64;  // lnb2
		ret = i2c_transfer(state->i2c, &msg, 1);
		if (ret!= 1)
		{
			dprintk(1, "%s: Error: LNB2 init3 A8292\n", __func__);
		}
		b = 0x82;  // lnb1
		ret = i2c_transfer(state->i2c, &msg, 1);
		if (ret != 1)
		{
			dprintk(1, "%s: Error:LNB1 init4 A8292\n", __func__);
		}
		b = 0x82 + 64;  // lnb2
		ret = i2c_transfer(state->i2c, &msg, 1);
		if (ret != 1)
		{
			dprintk(1, "%s: Error:LNB2 init5 A8292\n", __func__);
		}
	}
...
static int stv090x_setup(struct dvb_frontend *fe)
{
	...
	{
		dprintk(20, "Initializing STV0900\n");
	...
	{
		dprintk(20, "Initializing STV0903\n");
	...
	/* I2C repeater OFF */
	dprintk(20, "I2C repeater OFF\n");
	STV090x_SETFIELD_Px(reg, ENARPT_LEVEL_FIELD, config->repeater_level);
	dprintk(100, "STV090x_P1_I2CRPT: %04x\n", reg);  // 0x40
...
//	62 - frontend1
	...
	dprintk(100, "stv090x_write_reg(state, STV090x_P1_I2CRPT, %04x)\n",reg);
	...
	dprintk(100, "stv090x_write_reg(state, STV090x_NCOARSE, 0x13)\n");
	if (stv090x_write_reg(state, STV090x_NCOARSE, 0x13) < 0) /* set PLL divider */
	{
		goto err;
	}
	msleep(5);
	dprintk(100, "stv090x_write_reg(state, STV090x_I2CCFG, 0x08)\n");
	...
	if (state->internal->dev_ver >= 0x20)
	{
		...
		/* write cut20_val*/
		dprintk(20, "Setting up Cut 2.0 initial values\n");
		for (i = 0; i < t2_size; i++)
		{
	...
	else if (state->internal->dev_ver > 0x30)
	{
		...
		dprintk(1, "INFO: Cut %1x.%1x; probably incomplete support!\n", state->internal->dev_ver >> 8, state->internal->dev_ver & 0xff);
	}
	/* ADC1 range */
	reg = stv090x_read_reg(state, STV090x_TSTTNR1);
...
struct dvb_frontend *stv090x_attach(const struct stv090x_config *config, struct i2c_adapter *i2c, enum stv090x_demodulator demod)
{
	...
	&&  (state->demod_mode == STV090x_DUAL))
	{
		...
		dprintk(10, "Attaching %s demodulator(%d) Cut %1x.%1x\n", state->device == STV0900 ? "STV0900" : "STV0903", demod, state->internal->dev_ver >> 4, state->internal->dev_ver & 0x0f);
	...
	if (state->demod == STV090x_DEMODULATOR_0)
	{
		mutex_init(&demod_lock);mutex_init(&tuner_lock);
	}
	...
	dprintk(1, "Attaching %s demodulator(%d) Cut=0x%02x\n", state->device == STV0900 ? "STV0900" : "STV0903", demod, state->internal->dev_ver);
	return &state->frontend;
