...
static int tda10048_writereg(struct tda10048_state *state, u8 reg, u8 data)
{
	...
	int ret;
	u8 buf[] = { reg, data };
	struct i2c_msg msg = { .addr = config->demod_address, .flags = 0, .buf = buf, .len = 2 };

	dprintk(20, "%s(reg = 0x%02x, data = 0x%02x)\n", __func__, reg, data);

	ret = i2c_transfer(state->i2c, &msg, 1);

	if (ret != 1)
	{
		dprintk(1, "%s: writereg error (ret == %i)\n", __func__, ret);
	}
	return (ret != 1) ? -1 : 0;
}
...
static u8 tda10048_readreg(struct tda10048_state *state, u8 reg)
{
	...
	dprintk(20, "%s(reg = 0x%02x)\n", __func__, reg);
	ret = i2c_transfer(state->i2c, msg, 2);
	if (ret != 2)
	{
		dprintk(1, "%s: readreg error (ret == %i)\n", __func__, ret);
	}
	return b1[0];
}

static int tda10048_writeregbulk(struct tda10048_state *state, u8 reg, const u8 *data, u16 len)
{
	struct tda10048_config *config = &state->config;
	int ret = -EREMOTEIO;
	struct i2c_msg msg;
...
static int tda10048_set_phy2(struct dvb_frontend *fe, u32 sample_freq_hz, u32 if_hz)
{
...
static int tda10048_set_wref(struct dvb_frontend *fe, u32 sample_freq_hz, u32 bw)
{
	struct tda10048_state *state = fe->demodulator_priv;
	u64 t, z;
	u32 b = 8000000;
	...
	else if (bw == BANDWIDTH_7_MHZ)
	{
		b = 7000000;
	}
	else if (bw == BANDWIDTH_8_MHZ)
	{
		b = 8000000;
	}
	/* WREF = (B / (7 * fs)) * 2^31 */
...
static int tda10048_set_invwref(struct dvb_frontend *fe, u32 sample_freq_hz, u32 bw)
{
	struct tda10048_state *state = fe->demodulator_priv;
	u64 t;
	u32 b = 8000000;
...
static int tda10048_set_bandwidth(struct dvb_frontend *fe, enum fe_bandwidth bw)
{
	struct tda10048_state *state = fe->demodulator_priv;
	dprintk(100, "%s > (bw = %d)\n", __func__, bw);
...
static int tda10048_set_if(struct dvb_frontend *fe, enum fe_bandwidth bw)
{
	...
	switch (bw)
	{
		...
		default:
		{
			dprintk(1, "%s: CAUTION no default\n", __func__);
			return -EINVAL;
		}
	}
	...
	{
		dprintk(1, "%s: Incorrect attach settings\n", __func__);
		return -EINVAL;
	}
	dprintk(20, "- freq_if_hz = %d\n", state->freq_if_hz);
	dprintk(20, "- xtal_hz = %d\n", state->xtal_hz);
	dprintk(20, "- pll_mfactor = %d\n", state->pll_mfactor);
	dprintk(20, "- pll_nfactor = %d\n", state->pll_nfactor);
	dprintk(20, "- pll_pfactor = %d\n", state->pll_pfactor);

	/* Calculate the sample frequency */
	state->sample_freq = state->xtal_hz * (state->pll_mfactor + 45);
	state->sample_freq /= (state->pll_nfactor + 1);
	state->sample_freq /= (state->pll_pfactor + 4);
	dprintk(20, "- sample_freq = %d\n", state->sample_freq);
...
static int tda10048_firmware_upload(struct dvb_frontend *fe)
{
	...
	dprintk(10, "%s: Waiting for firmware upload (%s)...\n", __func__, TDA10048_DEFAULT_FIRMWARE);

	ret = request_firmware(&fw, TDA10048_DEFAULT_FIRMWARE, state->i2c->dev.parent);
	if (ret)
	{
		dprintk(1, "%s: Upload failed. (file not found?)\n", __func__);
		return -EIO;
	}
	else
	{
		dprintk(10, "%s: firmware read, %Zu bytes.\n", __func__, fw->size);
		ret = 0;
	}

	if (fw->size != TDA10048_DEFAULT_FIRMWARE_SIZE)
	{
		dprintk(1, "%s: Firmware size incorrect\n", __func__);
		ret = -EIO;
	}
	else
	{
		dprintk(10, "%s: Firmware uploading\n", __func__);

		/* Soft reset */
		tda10048_writereg(state, TDA10048_CONF_TRISTATE1, tda10048_readreg(state, TDA10048_CONF_TRISTATE1) & 0xfe);
		tda10048_writereg(state, TDA10048_CONF_TRISTATE1, tda10048_readreg(state, TDA10048_CONF_TRISTATE1) | 0x01);

		/* Put the demod into host download mode */
		tda10048_writereg(state, TDA10048_CONF_C4_1, tda10048_readreg(state, TDA10048_CONF_C4_1) & 0xf9);

		/* Boot the DSP */
		tda10048_writereg(state, TDA10048_CONF_C4_1, tda10048_readreg(state, TDA10048_CONF_C4_1) | 0x08);
		...
		while (pos < fw->size)
		{
			...
			tda10048_writeregbulk(state, TDA10048_DSP_CODE_IN, &fw->data[pos], cnt);
			pos += cnt;
		}
		ret = -EIO;
		...
		for (cnt = 0; cnt < 250 ; cnt += 10)
		{
			...
			if (tda10048_readreg(state, TDA10048_SYNC_STATUS) & 0x40)
			{
				ret = 0;
				break;
			}
	...
	if (ret == 0)
	{
		dprintk(10, "%s: Firmware uploaded\n", __func__);
		state->fwloaded = 1;
	}
	else
	{
		dprintk(1, "%s: Firmware upload failed\n", __func__);
	}
	return ret;
}
...
static int tda10048_set_inversion(struct dvb_frontend *fe, int inversion)
{
	...
	{
		tda10048_writereg(state, TDA10048_CONF_C1_1, tda10048_readreg(state, TDA10048_CONF_C1_1) | 0x20);
	}
	else
	{
		tda10048_writereg(state, TDA10048_CONF_C1_1, tda10048_readreg(state, TDA10048_CONF_C1_1) & 0xdf);
	}
	return 0;
}

/* Retrieve the demod settings */
static int tda10048_get_tps(struct tda10048_state *state, struct dvb_ofdm_parameters *p)
{
	u8 val;
...
static int tda10048_i2c_gate_ctrl(struct dvb_frontend *fe, int enable)
{
	...
	if (enable)
	{
		return tda10048_writereg(state, TDA10048_CONF_C4_1, tda10048_readreg(state, TDA10048_CONF_C4_1) | 0x02);
	}
	else
	{
		return tda10048_writereg(state, TDA10048_CONF_C4_1, tda10048_readreg(state, TDA10048_CONF_C4_1) & 0xfd);
	}
}
...
static int tda10048_set_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	struct tda10048_state *state = fe->demodulator_priv;

	dprintk(100, "%s > (frequency = %d)\n", __func__, p->frequency);
...
static int tda10048_read_ber(struct dvb_frontend *fe, u32 *ber)
{
	...
	*ber = tda10048_readreg(state, TDA10048_CBER_MSB) << 8 | tda10048_readreg(state, TDA10048_CBER_LSB);
	*ber = 0; // ???
	return 0;
}

static int tda10048_read_signal_strength(struct dvb_frontend *fe, u16 *signal_strength)
{
	struct tda10048_state *state = fe->demodulator_priv;
	u8 v;
...
static int tda10048_read_ucblocks(struct dvb_frontend *fe, u32 *ucblocks)
{
	...
	*ucblocks = tda10048_readreg(state, TDA10048_UNCOR_CPT_MSB) << 8 | tda10048_readreg(state, TDA10048_UNCOR_CPT_LSB);
	return 0;
}

static int tda10048_get_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	struct tda10048_state *state = fe->demodulator_priv;

	dprintk(100, "%s >\n", __func__);

	p->inversion = tda10048_readreg(state, TDA10048_CONF_C1_1) & 0x20 ? INVERSION_ON : INVERSION_OFF;
	return tda10048_get_tps(state, &p->u.ofdm);
}

static int tda10048_get_tune_settings(struct dvb_frontend *fe, struct dvb_frontend_tune_settings *tune)
{
...
struct dvb_frontend *tda10048_attach(const struct tda10048_config *config, struct i2c_adapter *i2c)
{
	...
	/* create dvb_frontend */
	memcpy(&state->frontend.ops, &tda10048_ops,	sizeof(struct dvb_frontend_ops));
