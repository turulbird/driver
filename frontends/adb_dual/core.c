...
static struct dvb_frontend *frontend_init(struct core_config *cfg, int i)
{
	...
	printk(KERN_INFO "[fe_core_twin] %s frontend_init >\n", __func__);
	...
	if (frontend)
	{
		printk("[fe_core_twin] STB0899 attached OK\n");

		if (i == 0)
		{
			if (dvb_attach(stb6100_attach, frontend, &stb6100_config_1, cfg->i2c_adap) == 0)
			{
				printk(KERN_INFO "[fe_core_twin] Error attaching STB6100(1)\n");
				goto error_out;
			}
		}
		else
		{
			...
			{
				printk(KERN_INFO "[fe_core_twin] Error attaching STB6100(2)\n");
				goto error_out;
			}
		}
		printk("[fe_core_twin] STB6100(%d) attached OK\n", i);
	}
	else
	{
		printk(KERN_INFO "[fe_core_twin] %s: Error attaching STB0899\n", __func__);
		goto error_out;
	}
	return frontend;

error_out:
	printk("[fe_core_twin] Frontend registration failed!\n");
	if (frontend)
	{
...
static struct dvb_frontend *init_fe_device(struct dvb_adapter *adapter, struct plat_tuner_config *tuner_cfg, int i)
{
	...
	printk("[fe_core_twin] %s: Call dvb_register_frontend (adapter = 0x%x)\n", __func__, (unsigned int) adapter);

	if (dvb_register_frontend(adapter, frontend))
	{
		printk ("[fe_core_twin] %s: Frontend registration failed !\n", __func__);
...
{
	printk("[fe_core_twin] %s Frontend core loaded\n", __func__);
	return 0;
}

static void __exit fe_core_exit(void)
{
   printk("[fe_core_twin] Frontend core unloaded\n");
}
