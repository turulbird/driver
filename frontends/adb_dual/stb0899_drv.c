...
#define I2C_ADDR_STB0899_1 (0xd0 >> 1)	// 0x68
#define I2C_ADDR_STB6100_1 (0xc0 >> 1)	// 0x60

#define I2C_ADDR_STB0899_2 (0xd2 >> 1)	// 0x69
#define I2C_ADDR_STB6100_2 (0xc4 >> 1)	// 0x62
...
static int pwm_wait_diseqc1_idle(int timeout)
{
	...
	while (1)
	{
		...
		{
			dprintk(&verbose, FE_ERROR, 1, "[stb0899_drv] %s: Tuner A DiSEqc idle timeout!\n", __func__);
			return -ETIMEDOUT;
		}
		msleep(10);
	};
...
static int pwm_wait_diseqc2_idle(int timeout)
{
	...
	while (1)
	{
		...
		if (jiffies - start > timeout)
		{
			dprintk(&verbose, FE_ERROR, 1, "[stb0899_drv] %s: Tuner B DiSEqc idle timeout!!\n", __FUNCTION__);
			return -ETIMEDOUT;
		}
		msleep(10);
	};
...
static int pwm_send_diseqc1_burst(struct dvb_frontend *fe, fe_sec_mini_cmd_t burst)
{
	...
	switch(burst)
	{
		case SEC_MINI_A:
		{
			dprintk(&verbose, FE_DEBUG, 1, "[stb0899_drv] %s Tuner A Tone = A\n", __func__);
			for (i = 0; i < 8; i++)
			{
				pwm_diseqc_buf1_len++;
		...
		case SEC_MINI_B:
		{
			dprintk(&verbose, FE_DEBUG, 1, "[stb0899_drv] %s Tuner A Tone = B\n", __func__);
			for (i = 0; i < 8; i++)
			{
				pwm_diseqc_buf1_len++;
				pwm_diseqc_buf1[pwm_diseqc_buf1_len] = 1;
...
static int pwm_send_diseqc2_burst(struct dvb_frontend *fe, fe_sec_mini_cmd_t burst)
{
	...
	switch(burst)
	{
		case SEC_MINI_A:
		{
			dprintk(&verbose, FE_DEBUG, 1, "[stb0899_drv] %s Tuner B Tone = A\n", __func__);
			for (i = 0; i < 8; i++)
			{
				pwm_diseqc_buf2_len++;
		...
		case SEC_MINI_B:
		{
			dprintk(&verbose, FE_DEBUG, 1, "[stb0899_drv] %s Tuner B Tone = B\n", __func__);
			for (i = 0; i < 8; i++)
			{
				pwm_diseqc_buf2_len++;
				pwm_diseqc_buf2[pwm_diseqc_buf2_len] = 1;
...
static int pwm_diseqc1_send_msg(struct dvb_frontend *fe, struct dvb_diseqc_master_cmd *m)
{
	...
	dprintk(&verbose, FE_DEBUG, 1, "[stb0899_drv] %s > (msg len = %d)\n", __func__, m->msg_len);
	...
	for (j = 0; j < m->msg_len; j++)
	{
		...
		unsigned char parity = 0;
 		dprintk(&verbose, FE_DEBUG, 1, "[stb0899_drv] Tuner A: Send DiSEqC byte 0x%02x\n", m->msg[j]);
		for (i = 0; i < 8; i++)
		{
			if ((byte & 128) == 128)
			{	//diseqc 1
...
static int pwm_diseqc2_send_msg(struct dvb_frontend *fe, struct dvb_diseqc_master_cmd *m)
{
	...
	for (j = 0; j < m->msg_len; j++)
	{
		...
		unsigned char parity = 0;
 		...
 		dprintk(&verbose, FE_DEBUG, 1, "[stb0899_drv] Tuner B: Send DiSEqC byte 0x%02x\n", m->msg[j]);
		for (i = 0; i < 8; i++)
		{
			if ((byte & 128) == 128)
			{	// diseqc 1
...
{
	dprintk(&verbose, FE_DEBUG, 1, "[stb0899_drv] PWM Diseqc Initialization\n");
	...
	{
		dprintk(&verbose, FE_ERROR, 1, "[stb0899_drv] FAIL: request pin 5,5\n");
		goto err;
	}
	stpio_set_pin(pin_tx_diseqc1, 0);
	...
	{
		dprintk(&verbose, FE_ERROR, 1, "[stb0899_drv] FAIL: request pin 2,5\n");
		goto err;
	}
	stpio_set_pin(pin_tx_diseqc2, 0);
	...
	{
		dprintk(&verbose, FE_ERROR, 1, "[stb0899_drv] FAIL: request irq pwm\n");
		goto err;
	}
	...
	dprintk(&verbose, FE_DEBUG, 1, "[stb0899_drv] PWM Diseqc Init completed succesfully\n");
	return 0;
...
static int _stb0899_read_reg(struct stb0899_state *state, unsigned int reg)
{
	...
	if (ret != 2)
	{
		if (ret != -ERESTARTSYS)
		{
			dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] Read error, Reg = [0x%02x], Status = %d", reg, ret);
		}
		return ret < 0 ? ret : -EREMOTEIO;
	}
	if (unlikely(*state->verbose > FE_DEBUGREG))
	{
		dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] Reg = [0x%02x], data = %02x",reg, buf);
	}
...
u32 _stb0899_read_s2reg(struct stb0899_state *state, u32 stb0899_i2cdev, u32 stb0899_base_addr, u16 stb0899_reg_offset)
{
	...
	if (status < 1)
	{
		...
		{
			printk(KERN_ERR "[stb0899_drv] %s ERR(1), Device = [0x%04x], Base address = [0x%08x], Offset = [0x%04x], Status = %d\n",
	...
	if (status < 1)
	{
		if (status != -ERESTARTSYS)
		{
			printk(KERN_ERR "[stb0899_drv] %s ERR(2), Device = [0x%04x], Base address = [0x%08x], Offset = [0x%04x], Status = %d\n",
			       __func__, stb0899_i2cdev, stb0899_base_addr, stb0899_reg_offset, status);
		}
		goto err;
	}
	...
	if (status < 1)
	{
		if (status != -ERESTARTSYS)
		{
			printk(KERN_ERR "[stb0899_drv] %s ERR(3), Device = [0x%04x], Base address = [0x%08x], Offset = [0x%04x], Status = %d\n",
			       __func__, stb0899_i2cdev, stb0899_base_addr, stb0899_reg_offset, status);
		}
	...
	if (unlikely(*state->verbose > FE_DEBUGREG))
	{
		printk(KERN_DEBUG "[stb0899_drv] %s Device = [0x%04x], Base address = [0x%08x], Offset = [0x%04x], Data = [0x%08x]\n",
...
int stb0899_write_s2reg(struct stb0899_state *state, u32 stb0899_i2cdev, u32 stb0899_base_addr, u16 stb0899_reg_offset, u32 stb0899_data)
{
	...
	if (unlikely(status < 1))
	{
		if (status != -ERESTARTSYS)
		{
			printk(KERN_ERR "[stb0899_drv] %s ERR (1), Device = [0x%04x], Base Address = [0x%08x], Offset = [0x%04x], Data = [0x%08x], status = %d\n",
	...
	if (unlikely(status < 1))
	{
		if (status != -ERESTARTSYS)
		{
			printk(KERN_ERR "[stb0899_drv] %s ERR (2), Device = [0x%04x], Base Address = [0x%08x], Offset = [0x%04x], Data = [0x%08x], status = %d\n",
			       __func__, stb0899_i2cdev, stb0899_base_addr, stb0899_reg_offset, stb0899_data, status);
		}
		return status < 0 ? status : -EREMOTEIO;
	}
...
int stb0899_read_regs(struct stb0899_state *state, unsigned int reg, u8 *buf, u32 count)
{
	...
	if (status != 2)
	{
		if (status != -ERESTARTSYS)
		{
			printk(KERN_ERR "[stb0899_drv] %s Read error, Reg = [0x%04x], Count = %u, Status = %d\n",
	...
	if (unlikely(*state->verbose > FE_DEBUGREG))
	{
		...
		printk(KERN_DEBUG "[stb0899_drv] %s [0x%04x]:", __func__, reg);
		for (i = 0; i < count; i++)
		{
			printk(" %02x", buf[i]);
		}
...
int stb0899_write_regs(struct stb0899_state *state, unsigned int reg, u8 *data, u32 count)
{
	...
	if (ret != 1)
	{
		if (ret != -ERESTARTSYS)
		{
			dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] Reg = [0x%04x], Data = [0x%02x ...], Count = %u, Status = %d",
...
static u32 stb0899_get_mclk(struct stb0899_state *state)
{
	...
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] div = %d, mclk = %d", div, mclk);
	return mclk;
}
...
static void stb0899_set_mclk(struct stb0899_state *state, u32 Mclk)
{
	...
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] state->config = %p", state->config);
	mdiv = ((6 * Mclk) / state->config->xtal_freq) - 1;
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] mdiv = %d", mdiv);

	stb0899_write_reg(state, STB0899_NCOARSE, mdiv);
	internal->master_clk = stb0899_get_mclk(state);

	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] MasterCLOCK = %d", internal->master_clk);
}
...
static void stb0899_release(struct dvb_frontend *fe)
{
	...
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Release frontend");
	/* post process event */
...
static int stb0899_wait_diseqc_fifo_empty(struct stb0899_state *state, int timeout)
{
	...
	while (1)
	{
		...
		if ((jiffies - start) > timeout)
		{
			dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] DiSEqC fifo empty timed out!");
			return -ETIMEDOUT;
		}
	}
	return 0;
...
static int stb0899_send_diseqc_msg(struct dvb_frontend *fe, struct dvb_diseqc_master_cmd *cmd)
{
	...
	dprintk(state->verbose, FE_INFO, 1, "[stb0899_drv] %s >\n");
	if (cmd->msg_len > 8)
	{
		return -EINVAL;
	}
...
static int stb0899_wait_diseqc_rxidle(struct stb0899_state *state, int timeout)
{
	...
	while (!STB0899_GETFIELD(RXEND, reg))
	{
		...
		if (jiffies - start > timeout)
		{
			dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] DiSEqC Rx timed out!");
			return -ETIMEDOUT;
		}
		msleep(10);
	}
...
static int stb0899_recv_slave_reply(struct dvb_frontend *fe, struct dvb_diseqc_slave_reply *reply)
{
	...
	dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] %s >", __func__);
...
static int stb0899_wait_diseqc_txidle(struct stb0899_state *state, int timeout)
{
	...
	while (!STB0899_GETFIELD(TXIDLE, reg))
	{
		...
		if (jiffies - start > timeout)
		{
			dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] DiSEqC Tx timed out!!");
			return -ETIMEDOUT;
		}
		msleep(10);
	}
...
#else
	dprintk(state->verbose, FE_INFO, 1, "[stb0899_drv] Going to Sleep");
...
static int stb0899_init(struct dvb_frontend *fe)
{
	...
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] i2c_addr:%.2x >> \n",state->config->demod_address);
...
//	mutex_init(&state->search_lock);
	...
	if (bsla_init == 0)
	{
		...
		if (ret != 1)
		{
			printk("[stb0899_drv] Error: ISL6405 1\n");
		}
		isl6405_tone_sr2 = 0;
		...
		if (ret != 1)
		{
			printk("Error: ISL6405 2\n");
		}
	}
	/* init device */
	dprintk(state->verbose, FE_INFO, 1, "[stb0899_drv] init STB0899 device");
	for (i = 0; config->init_dev[i].address != 0xffff; i++)
	{
		stb0899_write_reg(state, config->init_dev[i].address, config->init_dev[i].data);
	}
	dprintk(state->verbose, FE_INFO, 1, "[stb0899_drv] init S2 demod");
	/* init S2 demod */
	...
	dprintk(state->verbose, FE_INFO, 1, "[stb0899_drv] init S1 demod");
	/* init S1 demod */
	...
	dprintk(state->verbose, FE_INFO, 1, "[stb0899_drv] init S2 FEC");
	/* init S2 fec */
	...
	dprintk(state->verbose, FE_INFO, 1, "[stb0899_drv] init TST");
	/* init test */
			...
			if (internal->lock)
			{
				...
				if (STB0899_GETFIELD(VSTATUS_LOCKEDVIT, reg))
				{
					...
					*strength += 750;
					...
					dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] AGCIQVALUE = 0x%02x, C = %d * 0.1 dBm", val & 0xff, *strength);
				}
			}
			break;
		}
...
//				reg = STB0899_READ_S2REG(STB0899_DEMOD, IF_AGC_GAIN);
		...
		default:
		{
			dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] Unsupported delivery system");
			return -EINVAL;
		}
	}
	return 0;
			...
			if (internal->lock)
			{
				if (STB0899_GETFIELD(VSTATUS_LOCKEDVIT, reg))
				{
					...
					*snr = stb0899_table_lookup(stb0899_cn_tab, ARRAY_SIZE(stb0899_cn_tab) - 1, val);
					...
					dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] NIR = 0x%02x%02x = %u, C/N = %d * 0.1 dBm\n",
					buf[0], buf[1], val, *snr);
				}
			}
			break;
		...
		case SYS_DVBS2:
		{
			if (internal->lock)
			{
				...
				*snr = val;
				...
				dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Es/N0 quant = %d (%d) estimate = %u (%d), C/N = %d * 0.1 dBm", quant, quantn, est, estn, val);
			}
			break;
		}
		default:
...
static int stb0899_read_status(struct dvb_frontend *fe, enum fe_status *status)
{
	...
	*status = 0;
	...
	switch (state->delsys)
	{
		...
		case SYS_DSS:
		{
			dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Delivery system DVB-S/DSS");
			if (internal->lock)
			{
				reg  = stb0899_read_reg(state, STB0899_VSTATUS);
				if (STB0899_GETFIELD(VSTATUS_LOCKEDVIT, reg))
				{
					dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] FE_HAS_CARRIER | FE_HAS_LOCK");
					*status |= FE_HAS_SIGNAL | FE_HAS_CARRIER | FE_HAS_LOCK;

					reg = stb0899_read_reg(state, STB0899_PLPARM);
					if (STB0899_GETFIELD(VITCURPUN, reg))
					{
						dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] FE_HAS_VITERBI | FE_HAS_SYNC");
		...
		case SYS_DVBS2:
		{
			dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Delivery system DVB-S2");
			if (internal->lock)
			{
				...
				if (STB0899_GETFIELD(UWP_LOCK, reg) && STB0899_GETFIELD(CSM_LOCK, reg))
				{
					*status |= FE_HAS_SIGNAL | FE_HAS_CARRIER;
					dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] UWP & CSM Lock -> DVB-S2 FE_HAS_CARRIER");

					reg = stb0899_read_reg(state, STB0899_CFGPDELSTATUS1);
					if (STB0899_GETFIELD(CFGPDELSTATUS_LOCK, reg))
					{
						*status |= FE_HAS_LOCK;
						dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Packet Delineator Locked -> DVB-S2 FE_HAS_LOCK");
					}
					if (STB0899_GETFIELD(CONTINUOUS_STREAM, reg))
					{
						*status |= FE_HAS_VITERBI;
						dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Packet Delineator found VITERBI -> DVB-S2 FE_HAS_VITERBI");
					}
					if (STB0899_GETFIELD(ACCEPTED_STREAM, reg))
					{
						*status |= FE_HAS_SYNC;
						dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Packet Delineator found SYNC -> DVB-S2 FE_HAS_SYNC");
						/* post process event */
						stb0899_postproc(state, STB0899_POSTPROC_GPIO_LOCK, 1);
					}
				}
...
static int stb0899_set_voltage(struct dvb_frontend *fe, fe_sec_voltage_t voltage)
{
	...
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] i2c_addr:%.2x >> ",state->config->demod_address);
	...
	if (state->config->demod_address == I2C_ADDR_STB0899_1)
	{
		switch (voltage)
		{
			case SEC_VOLTAGE_13:
			{
				dprintk(state->verbose, FE_DEBUG, 1,"[stb0899_drv] SEC_VOLTAGE_13 POL:V");
				isl6405_vol_sr1 = 0;  // 13V
				b = isl6405_init_sr1 + isl6405_vol_sr1 + isl6405_tone_sr1;
				ret = i2c_transfer(state->i2c, &msg, 1);
				if (ret != 1)
				{
					dprintk(state->verbose, FE_DEBUG, 1,"[stb0899_drv] SEC_VOLTAGE_13 Error ISL6405");
				}
				break;
			}
			case SEC_VOLTAGE_18:
			{
				dprintk(state->verbose, FE_DEBUG, 1,"[stb0899_drv] SEC_VOLTAGE_18 POL:H");
				isl6405_vol_sr1=ISL6405_VSEL1;  // 18V
				b = isl6405_init_sr1 + isl6405_vol_sr1 + isl6405_tone_sr1;
				ret = i2c_transfer(state->i2c, &msg, 1);
				if (ret != 1)
				{
					dprintk(state->verbose, FE_DEBUG, 1,"[stb0899_drv] SEC_VOLTAGE_18 Error ISL6405");
				}
				break;
			}
			case SEC_VOLTAGE_OFF:
			{
				...
				if (ret != 1)
				{
					dprintk(state->verbose, FE_DEBUG, 1,"[stb0899_drv] SEC_VOLTAGE_OFF Error ISL6405");
				}
				break;
			}
			default:
	...
	if (state->config->demod_address == I2C_ADDR_STB0899_2)
	{
		switch (voltage)
		{
			case SEC_VOLTAGE_13:
			{
				...
				if (ret != 1)
				{
					dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] SEC_VOLTAGE_13 Error ISL6405");
				}
				break;
			}
			case SEC_VOLTAGE_18:
			{
				...
				if (ret != 1)
				{
					dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] SEC_VOLTAGE_18 Error ISL6405");
				}
				break;
			}
			case SEC_VOLTAGE_OFF:
			{
				dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] SEC_VOLTAGE_OFF");
				isl6405_vol_sr2 = 0;
				isl6405_tone_sr2 = 0;
				b = 0 + ISL6405_SR;
				ret = i2c_transfer(state->i2c, &msg, 1);
				if (ret != 1)
				{
					dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] SEC_VOLTAGE_OFF Error ISL6405");
				}
				break;
			}
			default:
...
static int stb0899_set_tone(struct dvb_frontend *fe, fe_sec_tone_mode_t tone)
{
	...
	switch (tone)
	{
		case SEC_TONE_ON:
		{
			dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] SEC_TONE_ON");
			...
			if (state->config->demod_address == I2C_ADDR_STB0899_1)
			{
				...
				if (ret != 1)
				{
					dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] Error:ISL6405 tone on");
				}
			}
			if (state->config->demod_address == I2C_ADDR_STB0899_2)
			{
		...
		case SEC_TONE_OFF:
		{
			...
			if (state->config->demod_address == I2C_ADDR_STB0899_1)
			{
				...
				if (ret != 1)
				{
					dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] Error:ISL6405 tone off");
				}
			}
			if (state->config->demod_address == I2C_ADDR_STB0899_2)
			{
...
int stb0899_i2c_gate_ctrl(struct dvb_frontend *fe, int enable)
{
	...
	if (enable)
	{
		dprintk(state->verbose, FE_DEBUG, 1,"[stb0899_drv] Enable");
	}
	else
	{
		dprintk(state->verbose, FE_DEBUG, 1,"[stb0899_drv] Disable");
	}
// fix for non-functioning tuners with a failed tuning message
	i2c_stat =  0x48 | 0x80;
...
err:
	dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] I2C Repeater control failed");
	return -EREMOTEIO;
}
...
int stb0899_i2c_gate_ctrl_old(struct dvb_frontend *fe, int enable)
{
	...
	if (enable)
	{
		dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Enabling I2C Repeater...");
	...
	{
		dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Disabling I2C Repeater...");
		i2c_stat &= ~STB0899_I2CTON;
		if (stb0899_write_reg(state, STB0899_I2CRPT, i2c_stat) < 0)
		{
			goto err;
...
int stb0899_get_dev_id(struct stb0899_state *state)
{
	...
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] ID reg = [0x%02x]", id);
	chip_id = STB0899_GETFIELD(CHIP_ID, id);
	release = STB0899_GETFIELD(CHIP_REL, id);

	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Device ID = [%d], Release = [%d]", chip_id, release);
	...
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Demodulator Core ID = [%s], Version = [%d]", (char *) &demod_str, demod_ver);
	...
	if (!(chip_id > 0))
	{
		dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] couldn't find a STB 0899");
		return -ENODEV;
	}
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] FEC Core ID=[%s], Version=[%d]", (char*) &fec_str, fec_ver);
	return 0;
}
...
static void stb0899_set_delivery(struct stb0899_state *state)
{
	...
	switch (state->delsys)
	{
		case SYS_DVBS:
		{
			dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Delivery System -- DVB-S");
			/* FECM/Viterbi ON */
		...
		case SYS_DSS:
		{
			dprintk(state->verbose, FE_DEBUG, 1, "Delivery System -- DSS");
			/* FECM/Viterbi ON */
			reg = stb0899_read_reg(state, STB0899_FECM);
...
static int stb0899_set_property(struct dvb_frontend *fe, struct dtv_property *tvp)
{
	...
	dprintk(state->verbose, FE_DEBUG, 1,"%s >", __func__);
	return 0;
}
...
static enum dvbfe_search stb0899_search(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	...
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] delivery system=%d", state->delsys);

	SearchRange = 10000000;
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Frequency=%d, Srate=%d", i_params->freq, i_params->srate);
	/* checking Search Range is meaningless for a fixed 3 Mhz			*/
	if (INRANGE(i_params->srate, 1000000, 45000000))
	{
		dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Parameters IN RANGE");
		...
		switch (state->delsys)
		{
			...
			case SYS_DSS:
			{
				dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] DVB-S delivery system");
				...
				dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] running DVB-S search algo...");
				if (stb0899_dvbs_algo(state) == RANGEOK)
				{
					internal->lock = 1;
					dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] -> DVB-S LOCK !");
					...
					dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] VSTATUS=0x%02x", internal->v_status);
					dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] ERR_CTRL=0x%02x", internal->err_ctrl);
#endif
...
//				pParams->SpectralInv = pSearch->IQ_Inversion;
				...
				/* Run the search algorithm */
				dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] running DVB-S2 search algo...");
				if (stb0899_dvbs2_algo(state) == DVBS2_FEC_LOCK)
				{
					internal->lock = 1;
					dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] -------------------------------------> DVB-S2 LOCK !");
...
static enum stb0899_status stb0899_track_carrier(struct stb0899_state *state)
{
	...
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] -> STB0899_DSTATUS=[0x%02x]", reg);
	if (STB0899_GETFIELD(CARRIER_FOUND, reg))
	{
		dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] -> CARRIEROK");
		return CARRIEROK;
	}
	else
	{
		dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] -> NOCARRIER");
		return NOCARRIER;
	}
	return NOCARRIER;
}
...
static enum stb0899_status stb0899_get_ifagc(struct stb0899_state *state)
{
	...
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] DMD_STATUS = [0x%02x]", reg);
	if (STB0899_GETFIELD(IF_AGC_LOCK, reg))
	{
		dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] -> IF AGC LOCKED");
		return AGC1OK;
	}
	else
	{
		dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] -> IF AGC LOCK LOST");
		return NOAGC1;
	}
	return NOAGC1;
}
...
static int stb0899_track(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	...
	switch (state->delsys)
	{
		...
		{
			dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Tracking DVB-S state");
		...
		{
			dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Tracking DSS state");
			if (stb0899_track_carrier(state) == CARRIEROK)
			{
				params->frequency			= internal->freq;
				params->inversion			= internal->inversion;
		...
		{
			dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Tracking DVB-S2 state");
			...
			dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Lock Lost=[0x%02x]\n", lock_lost);
			if (STB0899_GETFIELD(LOCK_LOST, lock_lost))
			{
				dprintk(state->verbose, FE_ERROR, 1, "[stb0899_drv] Demodulator LOST LOCK !\n");
			}
			break;
		}
		default:
...
static int stb0899_get_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	...
	dprintk(state->verbose, FE_DEBUG, 1, "[stb0899_drv] Get params");
...
struct dvb_frontend *stb0899_attach(struct stb0899_config *config, struct i2c_adapter *i2c)
{
	...
	{
		printk("[stb0899_drv] %s: Exiting!\n", __func__);
		goto error;
	}
	if (state->frontend.id == 0)
	{
		printk("[stb0899_drv] %s: Attaching STB0899 Tuner A\n", __func__);
	}
	else
	{
		printk("[stb0899_drv] %s: Attaching STB0899 Tuner B\n", __func__);
	}
