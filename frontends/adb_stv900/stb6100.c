...
static int stb6100_get_status(struct dvb_frontend *fe, u32 *status)
{
	...
	dprintk(verbose, FE_DEBUG, 1, "%s LOCK: %d", __func__, i);
	return i;
}
...
struct dvb_frontend *stb6100_attach(struct dvb_frontend *fe, struct stb6100_config *config, struct i2c_adapter *i2c)
{
	...
	dprintk(verbose, FE_DEBUG, 1, "Attaching STB6100");
	return fe;

error:
	dprintk(verbose, FE_ERROR, 1, "Error while attaching STB6100");
	kfree(state);
	return NULL;
}
