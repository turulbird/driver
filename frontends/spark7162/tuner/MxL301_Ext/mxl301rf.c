...

{
	...
	SetIRVBit(IRV_RFTune, 0x11, 0xFF, (u8)dig_rf_freq);
	SetIRVBit(IRV_RFTune, 0x12, 0xFF, (u8)(dig_rf_freq >> 8));
...
#else
			else if (((RF_Freq >= 689.5 * MHz) && (RF_Freq <= 690.5 * MHz))
			||       ((RF_Freq >= 713.5 * MHz) && (RF_Freq <= 714.5 * MHz))
		 	||       ((RF_Freq >= 737.5 * MHz) && (RF_Freq <= 738.5 * MHz)))
#endif
