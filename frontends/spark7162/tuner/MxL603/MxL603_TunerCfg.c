...
MXL_STATUS MxL603_Ctrl_ProgramRegisters(u32 devId, PMXL603_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
...
MXL_STATUS MxL603_Ctrl_WriteRegField(u32 devId, PMXL603_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
	MXL_STATUS status = MXL_TRUE;
	u8 tmp = 0;
	...
	tmp &= (u8)~ctrlRegInfoPtr->mask; // Clear the field that need to set value
	tmp |= (u8)(ctrlRegInfoPtr->data & ctrlRegInfoPtr->mask);
