...
YW_ErrorType_T D0367qam_ScanFreq(TUNER_IOREG_DeviceMap_t *DemodDeviceMap, IOARCH_Handle_t DemodIOHandle);
...
static int dvb_d0367_fe_qam_read_signal_strength(struct dvb_frontend *fe, u16 *strength)
{
	struct dvb_d0367_fe_qam_state *state = fe->demodulator_priv;
	TUNER_IOREG_DeviceMap_t *DeviceMap;
	IOARCH_Handle_t IOHandle;
...
static int dvb_d0367_fe_qam_read_ucblocks(struct dvb_frontend *fe, u32 *ucblocks)
{
	*ucblocks = 0;
	return 0;
}
...
static int dvb_d0367_fe_qam_get_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	return 0;
}

static int dvb_d0367_fe_qam_set_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	struct dvb_d0367_fe_qam_state *state = fe->demodulator_priv;
...
static int dvb_d0367_fe_qam_read_status(struct dvb_frontend *fe, fe_status_t *status)
{
	struct dvb_d0367_fe_qam_state *state = fe->demodulator_priv;
	//int iTunerLock = 0;
	BOOL bIsLocked;
...
#if 1
	{
		...
		FE_STV0367qam_GetSignalInfo(&state->DeviceMap, state->IOHandle, &Quality, &Intensity, &Ber,  FirstTimeBER);
...
static int dvb_d0367_fe_qam_get_info(struct dvb_frontend *fe, struct dvbfe_info *fe_info)
{
	//struct dvb_d0367_fe_qam_state* state = fe->demodulator_priv;
...
static void dvb_d0367_fe_qam_tuner_is_lock(struct dvb_frontend *fe, u32 *status)
{
	if (fe->ops.tuner_ops.get_status)
	{
...
static void D0367qam_TunerIsLock(TUNER_IOREG_DeviceMap_t *DeviceMap, u32 *status)
{
	struct dvb_d0367_fe_qam_state *state = (struct dvb_d0367_fe_qam_state *)DeviceMap->priv;
	struct dvb_frontend *fe = &state->frontend;

	dvb_d0367_fe_qam_tuner_is_lock(fe, status);
}

U32 D0367qam_GeFrequencyKhz(TUNER_IOREG_DeviceMap_t *DeviceMap, IOARCH_Handle_t IOHandle)
{
...
U32 D0367qam_GeSymbolRate(TUNER_IOREG_DeviceMap_t *DeviceMap, IOARCH_Handle_t IOHandle)
{
...
U32 D0367qam_GeModulation(TUNER_IOREG_DeviceMap_t *DeviceMap, IOARCH_Handle_t IOHandle)
{
...
void D0367qam_TunerSetFreq(TUNER_IOREG_DeviceMap_t *DeviceMap, IOARCH_Handle_t IOHandle)
{
	struct dvb_d0367_fe_qam_state *state = (struct dvb_d0367_fe_qam_state *)DeviceMap->priv;
...
struct dvb_frontend *dvb_d0367_fe_qam_attach(struct i2c_adapter *i2c)
{
...
#if 0
	{
		...
		FE_STV0367qam_GetSignalInfo(DeviceMap, IOHandle, &Quality, &Intensity, &Ber,  FirstTimeBER);
		printk("Quality = %d, Intensity = %d, Ber = %d\n", Quality, Intensity, Ber);
	}
#if 0
	{
