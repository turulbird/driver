...
#define dprintk(args...) \
	do { \
		if (debug) printk(KERN_ERR "[tda10023] " args); \
	} while (0)
...
static u8 tda10023_readreg(struct tda10023_state *state, u8 reg)
{
	...
	dprintk("%s: reg=0x%02x , result=0x%02x\n",__func__, reg, b1[0]);
	return b1[0];
}
