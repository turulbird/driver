...
/*
 * DVB-S(2) Tuner IC is a Sharp IX2470VA, connected as follows:
 * DVB-S(2)/DVB-T select (low is DVB-S(S): PIO2.5
 * DVBS2OE#                              : PIO3.3
 * DVBS2PWREN#                           : PIO4.6
 * Tuner RESET                           : PIO5.3
 *  
 * LNB driver is an STM LNBP12, connected as follows:
 *
...
static struct platform_frontend_s avl2108_config =
{
	...
	.frontendList = (struct platform_frontend_config_s[])
	{
		[0] =
		{
			...
			.tuner_enable = { 5, 3, 1 },  // group, bit, state 
			.lnb          = { 5, 2, 0, 2, 2, 1 },  // enable (group, bit, state for 13V), voltage select (group, bit, state)
			.i2c_bus      = 0,
