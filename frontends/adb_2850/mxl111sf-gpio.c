...
static int mxl111sf_config_gpio_pins(struct mxl111sf_state *state, struct mxl_gpio_cfg *gpio_cfg)
{
	...
	ret = (MXL_GPIO_DIR_OUTPUT == gpio_cfg->dir) ? mxl111sf_set_gpo_state(state, gpio_cfg->pin, gpio_cfg->val) : mxl111sf_get_gpi_state(state,  gpio_cfg->pin, &gpio_cfg->val);
	mxl_fail(ret);
...
static int mxl111sf_hw_do_set_gpio(struct mxl111sf_state *state, int gpio, int direction, int val)
{
...
static int pca9534_set_gpio(struct mxl111sf_state *state, int gpio, int val)
{
	...
	struct i2c_msg msg[] =
	{
		{ .addr = PCA9534_I2C_ADDR, .flags = 0, .buf = w, .len = 1 },
		{ .addr = PCA9534_I2C_ADDR, .flags = I2C_M_RD, .buf = &r, .len = 1 },
	};
...
static int pca9534_init_port_expander(struct mxl111sf_state *state)
{
	...
	struct i2c_msg msg =
	{
		.addr = PCA9534_I2C_ADDR, .flags = 0, .buf = w, .len = 2
	};
	mxl_debug("()");
...
int mxl111sf_set_gpio(struct mxl111sf_state *state, int gpio, int val)
{
	...
	switch (state->gpio_port_expander)
	{
		default:
		{
			mxl_printk(KERN_ERR, "gpio_port_expander undefined, assuming PCA9534");
			/* fall-thru */
		}
