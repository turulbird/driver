...
#define HARD_RESET(state) \
	...
	{ \
		...
		if (state->cfg->reset) \
		{ \
			state->cfg->reset(state->fe,1); \
			msleep(10); \
			state->cfg->reset(state->fe,0); \
			msleep(10); \
		} \
	} while (0)
...
static int dib0070_tune_digital(struct dvb_frontend *fe, struct dvb_frontend_parameters *ch)
{
	...
	if (*tune_state == CT_TUNER_START)
	{
		...
		if (state->current_rf != freq)
		{
			...
			dib0070_write_reg(state, 0x20, 0x0040 | 0x0020 | 0x0010 | 0x0008 | 0x0002 | 0x0001 | state->current_tune_table_index->tuner_enable);
	...
	else if (*tune_state == CT_TUNER_STEP_4)
	{
		...
		if (tmp != NULL)
		{
			...
			dib0070_write_reg(state, 0x0f, (0 << 15) | (1 << 14) | (3 << 12) | (tmp->wbd_gain_val << 9) | (0 << 8) | (1 << 7) | (state->current_tune_table_index->wbdmux << 0));
			state->wbd_gain_current = tmp->wbd_gain_val;
		}
		else
		{
			dib0070_write_reg(state, 0x0f, (0 << 15) | (1 << 14) | (3 << 12) | (6 << 9) | (0 << 8) | (1 << 7) | (state->current_tune_table_index->wbdmux << 0));
			state->wbd_gain_current = 6;
		}
		dib0070_write_reg(state, 0x06, 0x3fff);
		dib0070_write_reg(state, 0x07, (state->current_tune_table_index->switch_trim << 11) | (7 << 8) | (state->lna_match->lna_band << 3) | (3 << 0));
