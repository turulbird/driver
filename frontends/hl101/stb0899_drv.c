 ...
 * 201????? Spider-team     Adapted for use on HL101.
...
int stb0899_write_s2reg(struct stb0899_state *state, u32 stb0899_i2cdev, u32 stb0899_base_addr, u16 stb0899_reg_offset, u32 stb0899_data)
{
	...
	if (unlikely(paramDebug >= 100))
	{
		dprintk(1, "%s Device = [0x%04x], Base Address = [0x%08x], Offset = [0x%04x], Data = [0x%08x]\n", __func__, stb0899_i2cdev, stb0899_base_addr, stb0899_reg_offset, stb0899_data);
	}
	status = i2c_transfer(state->i2c, &msg_0, 1);
...
//					dprintk(20, "AGCIQVALUE = 0x%02x, C = %d * 0.1 dBm\n", val & 0xff, *strength);
		...
		case SYS_DVBS2:
		{
			if (internal->lock)
			{
				...
				*strength += 750;
				*strength = *strength << 4;
				dprintk(50, "IF_AGC_GAIN = 0x%04x, C = %d * 0.1 dBm\n", val & 0x3fff, *strength);
			}
			break;
		}
		default:
			...
			if (internal->lock)
			{
				if (STB0899_GETFIELD(VSTATUS_LOCKEDVIT, reg))
				{
					...
					*snr = stb0899_table_lookup(stb0899_cn_tab, ARRAY_SIZE(stb0899_cn_tab) - 1, val);
					...
					dprintk(50, "NIR = 0x%02x%02x = %u, C/N = %d * 0.1 dBm\n", buf[0], buf[1], val, *snr);
				}
			}
			break;
		}
		case SYS_DVBS2:
		{
			if (internal->lock)
			{
				...
				*snr = val;
				...
				dprintk(50, "Es/N0 quant = %d (%d) estimate = %u (%d), C/N = %d * 0.1 dBm\n", quant, quantn, est, estn, val);
			}
			break;
		}
		default:
...
static int stb0899_read_status(struct dvb_frontend *fe, enum fe_status *status)
{
	...
	*status = 0;
	...
	switch (state->delsys)
	{
		...
		case SYS_DSS:
		{
			dprintk(50, "Delivery system DVB-S/DSS\n");
			if (internal->lock)
			{
				reg  = stb0899_read_reg(state, STB0899_VSTATUS);
				if (STB0899_GETFIELD(VSTATUS_LOCKEDVIT, reg))
				{
					dprintk(50, "--------> FE_HAS_CARRIER | FE_HAS_LOCK\n");
					*status |= FE_HAS_SIGNAL | FE_HAS_CARRIER | FE_HAS_LOCK;

					reg = stb0899_read_reg(state, STB0899_PLPARM);
					if (STB0899_GETFIELD(VITCURPUN, reg))
					{
						dprintk(50, "--------> FE_HAS_VITERBI | FE_HAS_SYNC\n");
			...
			if (internal->lock)
			{
				...
				if (STB0899_GETFIELD(UWP_LOCK, reg) && STB0899_GETFIELD(CSM_LOCK, reg))
				{
					*status |= FE_HAS_SIGNAL | FE_HAS_CARRIER;
					dprintk(50, "UWP & CSM Lock ! ---> DVB-S2 FE_HAS_CARRIER\n");

					reg = stb0899_read_reg(state, STB0899_CFGPDELSTATUS1);
					if (STB0899_GETFIELD(CFGPDELSTATUS_LOCK, reg))
					{
						*status |= FE_HAS_LOCK;
						dprintk(50, "Packet Delineator Locked ! -----> DVB-S2 FE_HAS_LOCK\n");

					}
					if (STB0899_GETFIELD(CONTINUOUS_STREAM, reg))
					{
						*status |= FE_HAS_VITERBI;
						dprintk(50, "Packet Delineator found VITERBI ! -----> DVB-S2 FE_HAS_VITERBI\n");
					}
					if (STB0899_GETFIELD(ACCEPTED_STREAM, reg))
					{
						*status |= FE_HAS_SYNC;
						dprintk(50, "Packet Delineator found SYNC ! -----> DVB-S2 FE_HAS_SYNC\n");
						/* post process event */
						stb0899_postproc(state, STB0899_POSTPROC_GPIO_LOCK, 1);
					}
				}
...
static int stb0899_set_voltage(struct dvb_frontend *fe, fe_sec_voltage_t voltage)
{
	...
	switch (voltage)
	{
		case SEC_VOLTAGE_13:
		{
			dprintk(20, "Set voltage 13V (vertical)\n");
		...
		case SEC_VOLTAGE_18:
		{
			dprintk(20, "Set voltage 18V (horizontal)\n");
			stpio_set_pin(state->config->lnb_enable, VOLTAGE_ON);
...
int stb0899_i2c_gate_ctrl(struct dvb_frontend *fe, int enable)
{
	...
	if (enable)
	{
		dprintk(50, "Enabling I2C Repeater...\n");
	...
	{
		dprintk(50, "Disabling I2C Repeater...\n");
		i2c_stat &= ~STB0899_I2CTON;
		if (stb0899_write_reg(state, STB0899_I2CRPT, i2c_stat) < 0)
		{
			goto err;
...
err:
	dprintk(50, "I2C Repeater control failed\n");
	return -EREMOTEIO;
}
...
int stb0899_get_dev_id(struct stb0899_state *state)
{
	...
	dprintk(100, "ID reg = [0x%02x]\n", id);
	chip_id = STB0899_GETFIELD(CHIP_ID, id);
	release = STB0899_GETFIELD(CHIP_REL, id);

	dprintk(50, "Device ID = [%d], Release = [%d]", chip_id, release);
	...
	dprintk(50, "Demodulator Core ID = [%s], Version = [%d]\n", (char *)&demod_str, demod_ver);
	...
	dprintk(50, "FEC Core ID = [%s], Version = [%d]\n", (char *) &fec_str, fec_ver);
	return 0;
}
...
static enum dvbfe_search stb0899_search(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	...
	dprintk(50, "delivery system = %d\n", state->delsys);

	SearchRange = 10000000;
	dprintk(50, "Frequency = %d, Srate = %d", i_params->freq, i_params->srate);
	/* checking Search Range is meaningless for a fixed 3 MHz */
	if (INRANGE(i_params->srate, 1000000, 45000000))
	{
		dprintk(50, "Parameters IN RANGE\n");
...
//	state->verbose                   = &verbose;
	...
	dprintk(50, "%s < STB0899 Forintend attached\n", __func__);
