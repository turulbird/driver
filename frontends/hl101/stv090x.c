 ...
 *
 * Version for:
 * Sipderbox HD HL-101 (STV0900) Tuner STV6110X
 *
...
static enum stv090x_signal_state stv090x_algo(struct stv090x_state *state)
{
	...
	{
		dprintk(1, "No Signal: POWER_IQ=0x%02x", power_iq);
		lock = 0;
...
static enum dvbfe_search stv090x_search(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	...
	if (state->srate > 10000000)
	{
		dprintk(50, "Search range: 10 MHz\n");
		state->search_range = 10000000;
	}
	else
	{
		dprintk(50, "Search range: 5 MHz\n");
...
static int stv090x_set_voltage(struct dvb_frontend *fe, fe_sec_voltage_t voltage)
{
	...
	switch (voltage)
	{
		case SEC_VOLTAGE_13:
		{
			dprintk(20, "Frontend %d: set voltage_13V (vertical)\n", fe->id);
		...
		case SEC_VOLTAGE_18:
		{
			dprintk(20, "Ffrontend %d: set voltage 18V (horizontal)\n", fe->id);
			stpio_set_pin(state->config->lnb_enable, VOLTAGE_ON);
		...
		{
			dprintk(20, "Frontend %d: set voltage_off\n", fe->id);
...
//	state->verbose                   = &verbose;
	...
	dprintk(20, "Attaching %s demodulator(%d) Cut=0x%02x", state->device == STV0900 ? "STV0900" : "STV0903", demod, state->internal->dev_ver);
	return &state->frontend;
