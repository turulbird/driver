...
#define UC_INTFC_CTRL_REG       0x1d
#define UC_WAITACK_CTRL_REG     0x1e
...
static int starci2_writereg(int reg, int data)
{
	...
	dprintk(1, "%s > reg=0x%02x, value=0x%02x\n", __func__, reg, data);
	if ((err = i2c_transfer(cicam.i2c, &msg, 1)) != 1)
	{
		dprintk(1, "%s: writereg error(err == %i, reg == 0x%02x, data == 0x%02x)\n", __func__, err, reg, data);
		return -EREMOTEIO;
	}
#endif
	return 0;
...
# if defined(IPBOX9900)
	...
	{
		printk ("%s: Error: Cannot find i2c adapter #0\n", __func__);
		return -ENODEV;
	}
	...
	dprintk(20, "%s: found supported device on %d@%x\n", __func__, STARCI2_I2C_BUS, STARCI2_ADDR);
#endif
	return 0;
}
...
int ciintf_set_twin_mode(int on, int locked)
{
	...
	if (on)
	{
		...
		starci2_writereg(SINGLE_MODE_CTRL_REG, 0x00);  // TS PATH
		starci2_writereg(TWIN_MODE_CTRL_REG, 0x80);  // TS PATH: TWIN ON, TSI1->1st TSI, TSI2->2nd TSI
		dprintk(1, "CICAM: TWIN mode set\n");
	}
	else
	{
		// SINGLE MODE
		...
		cicam.slot.output[1] = 1;
		starci2_writereg(SINGLE_MODE_CTRL_REG, 0x00);  // TS PATH
		starci2_writereg(TWIN_MODE_CTRL_REG, 0x00);  // TS PATH: TWIN OFF
		dprintk(1, "CICAM: SINGLE mode set\n");
	}
...
int ciintf_init(struct dvb_ca_en50221 *ca)
{
	...
	starci2_writereg(STARCI_CTRL_REG, 0x80);  // RESET ON
	msleep(10); //FIXME: better value?
	starci2_writereg(STARCI_CTRL_REG, 0x00);  // RESET OFF
	
	starci2_writereg(MODA_MASK_HIGH_REG, 0x00);  // MOD-A AUTOSELECT MSK HI
	starci2_writereg(MODA_MASK_LOW_REG, 0x01);  // MOD-A AUTOSELECT MSK LO
	starci2_writereg(MODA_PATTERN_HIGH_REG, 0x00);  // MOD-A AUTOSELECT PATT HI
	starci2_writereg(MODA_PATTERN_LOW_REG, 0x00);  // MOD-A AUTOSELECT PATT LO

	starci2_writereg(MODB_MASK_HIGH_REG, 0x00);  // MOD-B AUTOSELECT MSK HI
	starci2_writereg(MODB_MASK_LOW_REG, 0x01);  // MOD-B AUTOSELECT MSK LO
	starci2_writereg(MODB_PATTERN_HIGH_REG, 0x00);  // MOD-B AUTOSELECT PATT HI
	starci2_writereg(MODB_PATTERN_LOW_REG, 0x01);  // MOD-B AUTOSELECT PATT LO

	starci2_writereg(DEST_SEL_REG, 0x01);  // SET AUTO MODULE SELECT
	starci2_writereg(POWER_CTRL_REG, 0x00);  // SET PINS (power off)

	starci2_writereg(MODA_TIMING_REG, 0x40);  // MOD-A AM,CM TIMING

	starci2_writereg(MODB_TIMING_REG, 0x40);  // MOD-B AM,CM TIMING

	starci2_writereg(INT_MASK_REG, 0x00);  // MASK ALL INTERRUPTS
	starci2_writereg(UC_INTFC_CTRL_REG, 0x01);  // RD,DIR
	starci2_writereg(UC_WAITACK_CTRL_REG, 0x03);  // WAIT BUFF

	cicam.slot.mode = 0;  // faked to force processing on next line
	ciintf_set_twin_mode(1, 1);

	starci2_writereg(STARCI_CTRL_REG, 0x01);  // LOCK CONFIG

	//starci2_writereg(MODA_CTRL_REG, 0x00);  // MOD-A CTRL, AUTOACTIVATION OFF
	starci2_writereg(MODA_CTRL_REG, 0x02);  // MOD-A CTRL, AUTOACTIVATION ON

	//starci2_writereg(MODB_CTRL_REG, 0x00);  // MOD-B CTRL, AUTOACTIVATION OFF
	starci2_writereg(MODB_CTRL_REG, 0x02);  // MOD-B CTRL, AUTOACTIVATION ON

	starci2_writereg(POWER_CTRL_REG, 0x21); // POWER ON
	//
	// --- remapping stuffs ---
	//
...
#define EMIBufferBaseAddress (EMIConfigBaseAddress+0x800)
	...
	dprintk(1, "CI bank2 mapped %lX -> %lX\n", res, cicam.reg.bank2);
	...
	// CI power
	{
		...
		{
			dprintk(1, "ERR: Cannot acquire CI power pin2.6!\n");
		}
		else
		{
			dprintk(20, "CI power pin 2.6 acquired OK\n");
			stpio_set_pin(pwr, 1);
		}
		if ((pwr = stpio_request_pin(2, 7, "CIpwr2", STPIO_OUT)) == NULL)
		{
			dprintk(1, "ERR: Cannot acquire CI power pin2.7!\n");
		}
		else
		{
			dprintk(20, "CI power pin 2.7 acquired OK\n");
			stpio_set_pin(pwr,1);
		}
	}
	mutex_unlock(&cicam.lock);
...
static int ciintf_read_attribute_mem_locked(struct dvb_ca_en50221 *ca, int slot, int address)
{
	...
	val = starci2_readreg(real_slot == 0 ? MODA_CTRL_REG : MODB_CTRL_REG); // MOD-A CTRL
	if (val < 0)
	{
		return 0;
	}
...
#endif
	starci2_writereg(real_slot == 0 ? MODA_CTRL_REG : MODB_CTRL_REG, val & 0xF3);  // MOD-A CTRL: access to attribute mem
	res = ctrl_inb(cicam.reg.bank2 + (BANKMEM_OFFSET*real_slot) + (address << 0));
	//res &= 0xFF;

	if (address <= 2)
	{
		dprintk(1, "ATTRMEM#%d: address = %lx(%u): res = %x\n", real_slot, cicam.reg.bank2 + (BANKMEM_OFFSET*real_slot) + (address << 1), address, res);
	}
	else
	{
		if (res > 31 && res < 127)
...
#endif
	starci2_writereg(real_slot == 0 ? MODA_CTRL_REG : MODB_CTRL_REG, 0x63);  // MOD-A CTRL: access to attribute mem
...
#endif
	starci2_writereg(real_slot == 0 ? MODA_CTRL_REG : 0x09, (val & 0xF3) | 0x04);  // MOD-A CTRL: access to i/o mem
	res = ctrl_inb(cicam.reg.bank2  + (BANKMEM_OFFSET*real_slot) + (address << 0));
	res &= 0xFF;
	mutex_unlock(&cicam.lock);
#endif
...
int ciintf_slot_reset(struct dvb_ca_en50221 *ca, int slot)
{
	...
	res = starci2_readreg(slot == 0 ? MODA_CTRL_REG : MODB_CTRL_REG);
	if (res & 0x01)
	{
		starci2_writereg(slot == 0 ? MODA_CTRL_REG : MODB_CTRL_REG, res | 0x80);  // MOD-A CTRL
		msleep(60);
		dprintk(1, "RESET module %d\n", slot);
		starci2_writereg(slot == 0 ? MODA_CTRL_REG : MODB_CTRL_REG, 0x02);  // MOD-A CTRL
...
int ciintf_slot_ts_enable(struct dvb_ca_en50221 *ca, int slot)
{
	...
	starci2_writereg(real_slot == 0 ? MODA_CTRL_REG : MODB_CTRL_REG, 0x62);  // MPEG on, no bypass
	dprintk(1, "[%s] slot %d\n", __func__, real_slot);
	mutex_unlock(&cicam.lock);
#endif
	return 0;
...
int ciintf_poll_slot_status(struct dvb_ca_en50221 *ca, int slot, int open)
{
	...
	debug_ori = paramDebug;
	paramDebug = 255;  // DISABLE DEBUG
	slot_status = starci2_readreg(slot == 0 ? MODA_CTRL_REG : MODB_CTRL_REG) & 0x01;  // MOD-A CTRL
	paramDebug = debug_ori;
	...
	if (slot_status == 1)  // 1 = module detected
	{
		...
		if(cicam.slot_state[slot] & SLOT_STATUS_NONE)
		{
			...
			{
				// detected module insertion, set the detection timeout (500 ms)
		...
		if (cicam.slot_state[slot] & SLOT_STATUS_RESET)
		{
			...
			{
				dprintk(1, "DEB: >>> slot %d: DVB_CA_EN50221_POLL_CAM_READY\n", slot);
...
#if 0
	dprintk(20, "Module %d: present = %d, ready = %d\n", slot, slot_status & DVB_CA_EN50221_POLL_CAM_PRESENT, slot_status & DVB_CA_EN50221_POLL_CAM_READY);
#endif
	mutex_unlock(&cicam.lock);
#endif
	return slot_status;
...
int setInputSource(int pti, int source)
{
	...
	if (pti == 0)
	{
 		switch (source)
		{
			case 0: 
		 	{
				dprintk(1, "[%s] TUNER_A->TSIN0\n",__func__);
				break;
			}
			case 1: 
			{
		 		dprintk(1, "[%s] TUNER_B->TSIN0\n",__func__);
				break;
			}
			case 2: 
			{
		 		dprintk(1, "[%s] CI0->TSIN0\n",__func__);
				break;
			}
			case 3: 
			{
		 		dprintk(1, "[%s] CI1->TSIN0\n",__func__);
				break;
			}
		}
	 }
	else
	{
 		switch (source)
		{
 			case 0: 
			{
				dprintk(1, "[%s] TUNER_A->TSIN1\n",__func__);    			
				break;
			}
			case 1: 
			{
				dprintk(1, "[%s] TUNER_B->TSIN1\n",__func__);    			
				break;
			}
 			case 2: 
			{
				dprintk(1, "[%s] CI0->TSIN1\n",__func__);    			
				break;
			}
 			case 3: 
			{
				dprintk(1, "[%s] CI1->TSIN1\n",__func__);    			
				break;
			}
		 }
	 }
...
int setCiSource(int slot, int source)
{
	...
	val = starci2_readreg(TWIN_MODE_CTRL_REG);  // TWIN MODE TS CONTROL REG
	if (val < 0)
	{
		return 0;
	}
	switch (source)
	{
		case -1:
		{
			dprintk(20, "[%s] CI%d->bypass\n",__func__, slot);
			val |= slot == 0 ? 0x10 : 0x08;
			starci2_writereg(TWIN_MODE_CTRL_REG, val);
			break;
		}
		case 0: 
		case 1: 
		{
			dprintk(20, "[%s] TUNER_%c->CI%d\n",__func__, source ? 'B' : 'A', slot);
			...
			dprintk(20, "setCiSource: new ctrlx11=0x%x\n", val);
			starci2_writereg(TWIN_MODE_CTRL_REG, val);
			break;
		}
		case 2: 
		{
			dprintk(20, "[%s] CI0->CI%d\n",__func__, slot);
			break;
		}
		case 3: 
		{
			dprintk(20, "[%s] CI1->CI%d\n",__func__, slot);
			break;
		}
	}
#endif
...
{	
	...
	if (ciintf_detect())
	{
		dprintk(1, "CICAM chip not detected.\n");
	}
	else
	{
		...
		if (result != 0)
		{
			dprintk(1, "CICAM detected, but registration failed (errno = %d).\n", result);
			//goto error;
			return result;
		}
		else
		{
			...
			if (result != 0)
			{
				dprintk(1, "CICAM initialisation failed (errno = %d).\n", result);
				return result;
			}
			else
			{
				dprintk(20, "CICAM initialisation OK.\n");
			}
		}
	} // autodetect
	return 0;
...
{
	dprintk(0, "starci2win loaded\n");
	return 0;
}

void starci2win_exit(void)
{  
	 dprintk(0, "starci2win unloaded\n");
}
...
MODULE_PARM_DESC(paramDebug, "Debug Output 0=always >0=enabled(debuglevel)");
