...
enum
{
	...
	YWHAL_ERROR_NOT_ENOUGH_DEVICE                                          /* Device is already opened   */
};
...
#define YWI64_Add(A,B,Value) \
{ \
	register long long T1,T2,Val; \
...
#define YWI64_AddLit(A,B,Value) \
{ \
	register long long T1,Val; \
	T1 = (long long)(A).MSW << 32 | (A).LSW; \
	Val=T1+(B); \
	(Value).MSW = Val >> 32; \
	(Value).LSW = (U32)Val; \
...
/*A >= B, A & B are U64 type */
#define YWI64_IsGreaterOrEqual(A,B)       (((A).MSW >  (B).MSW) || (((A).MSW == (B).MSW) && ((A).LSW >= (B).LSW)))

/*A > B, A & B are U64 type */
#define YWI64_IsGreaterThan(A,B)          (((A).MSW >  (B).MSW) || (((A).MSW == (B).MSW) && ((A).LSW > (B).LSW)))

/*A < B, A & B are U64 type */
#define YWI64_IsLessThan(A,B)             (((A).MSW <  (B).MSW) || (((A).MSW == (B).MSW) && ((A).LSW < (B).LSW)))

/* A <= B, A & B are U64 type */
#define YWI64_IsLessOrEqual(A,B)          (((A).MSW <  (B).MSW) || (((A).MSW == (B).MSW) && ((A).LSW <= (B).LSW)))
...
#define YWI64_Sub(A,B,Value)              ((Value).MSW  = (A).MSW - (B).MSW - (((A).LSW < (B).LSW)?1:0), (Value).LSW  = (A).LSW - (B).LSW)

/* Value = A - B, where A is U64 type & B is 32-bit atmost */
#define YWI64_SubLit(A,B,Value)           ((Value).MSW  = (A).MSW - (((A).LSW < (B))?1:0), (Value).LSW  = (A).LSW - (B))

/* Value =A / B, where A, B is U64 type */
#define YWI64_Div(A,B,Value) \
{ \
	register long long T1,T2, Val; \
	T1 = (long long)(A).MSW << 32 | (A).LSW; \
	T2 = (long long)(B).MSW << 32 | (B).LSW; \
...
#define YWI64_ShiftLeft(Shift,Value) \
{ \
	register long long T1, T2, Val; \
...
/* Value = Value >> Shift, where Value is U64 type */
#define YWI64_ShiftRight(Shift,Value) \
{ \
	register long long T1, T2, Val; \
	T1 = (long long)(Value).MSW << 32 | (Value).LSW; \
	Val=T1 >> Shift; \
